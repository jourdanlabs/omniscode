...
fn setup_hint_test_env_lock() -> std::sync::MutexGuard<'static, ()> {
...
#[cfg(target_os = "macos")]
#[test]
fn configured_inherited_hotkeys_do_not_create_a_startup_notice() {
    let state = SetupHintsState {
        launch_count: 2,
...
fn baked_repo_hotkey_cds_into_fixed_dir() {
    ...
    let config = jcode_config_types::LaunchHotkeysConfig {
        ...
        entries: vec![jcode_config_types::LaunchHotkeyEntry {
            chord: "cmd+[".to_string(),
            dir: "/Users/example/omnis-key".to_string(),
    ...
    assert!(cmd.contains("/Users/example/omnis-key"));
...
fn should_record_last_dir_skips_home_only() {
    ...
    assert!(super::should_record_last_dir(
        Path::new("/Users/example/projects/foo"),
        Some(home)
    ));
...
#[test]
fn inherited_installed_runtime_callbacks_are_inert_and_zero_write() {
    let _lock = setup_hint_test_env_lock();
    let temp = tempfile::tempdir().expect("create hostile setup-hint home");
    let _home = ScopedJcodeHome::set(temp.path());
    let state_path = temp.path().join("setup_hints.json");
    std::fs::write(
        &state_path,
        r#"{"launch_count":41,"launch_hotkey_usage":{"cmd+;":7}}"#,
    )
    .expect("write setup-hint sentinel");
    let before = snapshot_tree(temp.path());

    record_launch_dirs(
        std::path::Path::new("/hostile/project"),
        Some(std::path::Path::new("/hostile/repository")),
    );
    record_launch_hotkey_use("cmd+;");
    reinstall_launch_hotkeys_after_config_change();
    run_setup_hotkey(false, false, false, Some("claude"))
        .expect("stale SessionStart callback should retire successfully");
    run_macos_hotkey_listener_main_thread()
        .expect("stale LaunchAgent listener should retire successfully");

    let setup_error = run_setup_hotkey(false, false, false, None)
        .expect_err("explicit inherited hotkey installation must remain disabled");
    assert!(
        setup_error
            .to_string()
            .contains("OMNIS_KEY_INHERITED_INSTALLER_DISABLED")
    );
    let launcher_error =
        run_setup_launcher().expect_err("explicit inherited launcher installation must fail");
    assert!(
        launcher_error
            .to_string()
            .contains("OMNIS_KEY_INHERITED_INSTALLER_DISABLED")
    ...
    assert_eq!(
        ...
        before,
        "retired installed-runtime callbacks mutated setup-hint state"
    );
    assert!(
        !temp.path().join("hotkey").exists(),
        "retired callback created launch-hotkey state"
    );
}

...
#[cfg(any(test, target_os = "macos"))]
use macos_terminal::{MacTerminalKind, effective_macos_terminal, save_preferred_macos_terminal};
#[cfg(windows)]
use windows_setup::{
    maybe_show_windows_setup_hints, run_setup_hotkey_windows, run_windows_hotkey_listener,
...
const INHERITED_INSTALLER_DISABLED_MESSAGE: &str = "OMNIS_KEY_INHERITED_INSTALLER_DISABLED: inherited Jcode launcher installation is disabled in OMNIS KEY Local Integrity V1";

fn inherited_installer_disabled() -> bool {
    true
}
...
pub fn record_launch_dirs(dir: &std::path::Path, repo_dir: Option<&std::path::Path>) {
    if inherited_installer_disabled() {
        return;
    }
...
pub fn run_setup_hotkey(
    ...
    if notify_cli_launch.is_some() {
        return Ok(());
    }
    anyhow::bail!(INHERITED_INSTALLER_DISABLED_MESSAGE)
}
...
pub fn run_macos_hotkey_listener_main_thread() -> Result<()> {
    // A previously installed LaunchAgent can survive an application upgrade.
    // Exit successfully before touching the run loop, launch plan, filesystem,
    // or child processes so launchd does not repeatedly restart a failing
    // retired helper.
    if inherited_installer_disabled() {
        return Ok(());
    }

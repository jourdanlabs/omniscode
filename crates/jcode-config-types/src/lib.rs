...
/// Retired update-channel compatibility value.
///
/// OMNIS KEY Local Integrity V1 parses this inherited field but performs no
/// update discovery, download, or self-replacement.
...
pub struct TerminalConfig {
    ...
    /// Terminal used by explicit in-app session spawns on macOS.
    ///
    ...
    /// `~/.jcode/preferred_terminal.json` file. OMNIS KEY V1 does not ship the
    /// inherited global-hotkey installer.
    ///
...
///
/// Optional third-party tool discovery.
///
/// OMNIS KEY never ships a discovery service endpoint. Both `enabled = true`
/// and an explicit user-supplied endpoint are required before the
/// `discover_tools` tool can be registered.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
#[serde(default)]
pub struct SponsorsConfig {
    /// Enable tool discovery. Disabled by default.
    pub enabled: bool,
...
pub struct FeatureConfig {
    ...
    /// Retired compatibility value; OMNIS KEY V1 performs no update behavior.

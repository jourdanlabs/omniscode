...
mod tests {
    ...
    fn folds_subdirectories_into_repo_root() {
        ...
        let locs = vec![
            SessionLocation::new("/u/fixture-user/proj", Some(ts(2026, 6, 24))),
            SessionLocation::new("/u/fixture-user/proj/crates/a", Some(ts(2026, 6, 24))),
            SessionLocation::new("/u/fixture-user/proj/crates/b", Some(ts(2026, 6, 23))),
        ];
        let ranked = rank_repositories_with(
            &locs,
            now,
            &RankOptions::default(),
            fake_resolver(&["/u/fixture-user/proj"]),
        );
        assert_eq!(ranked.len(), 1);
        assert_eq!(ranked[0].path, "/u/fixture-user/proj");
    ...
    fn excluded_paths_are_dropped() {
        let now = ts(2026, 6, 25);
        let locs = vec![
            SessionLocation::new("/home/example", Some(ts(2026, 6, 24))),
            SessionLocation::new("/home/example/work", Some(ts(2026, 6, 24))),
        ];
        let opts = RankOptions {
            excluded_paths: vec![PathBuf::from("/home/example")],
            ..RankOptions::default()
        };
        let ranked = rank_repositories_with(
            &locs,
            now,
            &opts,
            fake_resolver(&["/home/example", "/home/example/work"]),
        );
        assert_eq!(ranked.len(), 1);
        assert_eq!(ranked[0].path, "/home/example/work");
    }
    ...
    fn plan_assigns_top_repo_home_then_next_repos() {
        let ranked = vec![
            repo("/u/fixture-user/jcode", 600.0),
            repo("/u/fixture-user/scrollwm", 100.0),
            repo("/u/fixture-user/sideproj", 50.0),
            repo("/u/fixture-user/fourth", 10.0),
            repo("/u/fixture-user/fifth", 5.0),
        ];
        let plan = build_launch_hotkey_plan(
            Path::new("/u/fixture-user"),
            &ranked,
        ...
        assert_eq!(plan[0].dir, "/u/fixture-user/jcode");
        assert_eq!(plan[1].chord, "cmd+'");
        assert_eq!(plan[1].dir, "/u/fixture-user");
        assert_eq!(plan[1].label, "home");
        assert_eq!(plan[2].chord, "cmd+[");
        assert_eq!(plan[2].dir, "/u/fixture-user/scrollwm");
        assert_eq!(plan[3].chord, "cmd+]");
        assert_eq!(plan[3].dir, "/u/fixture-user/sideproj");
        assert_eq!(plan[4].chord, "cmd+\\");
        assert_eq!(plan[4].dir, "/u/fixture-user/fourth");
    }

    #[test]
    fn plan_skips_home_if_it_appears_in_ranking() {
        let ranked = vec![
            repo("/u/fixture-user", 999.0), // home ranked #1, should not take a repo slot
            repo("/u/fixture-user/jcode", 600.0),
        ];
        let plan = build_launch_hotkey_plan(
            Path::new("/u/fixture-user"),
    ...
    fn plan_truncates_to_available_repos() {
        let ranked = vec![repo("/u/fixture-user/only", 600.0)];
        let plan = build_launch_hotkey_plan(
            Path::new("/u/fixture-user"),
            &ranked,
            &DEFAULT_LAUNCH_HOTKEY_CHORDS,
        ...
        assert_eq!(plan[0].dir, "/u/fixture-user/only");
        assert_eq!(plan[1].dir, "/u/fixture-user");
    }
    ...
    fn plan_with_no_repos_still_binds_home() {
        ...
        assert_eq!(plan[0].chord, "cmd+;");
        assert_eq!(plan[0].dir, "/u/fixture-user");
    ...
    fn collect_reads_working_dirs_and_skips_junk() {
        ...
        write(
            "session_a.json",
            r#"{"working_dir":"/Users/example/project","id":"a"}"#,
        );
        write(
            "session_b.json",
            r#"{"working_dir":"/Users/example/other-project","id":"b"}"#,
        );
        ...
        assert_eq!(
            dirs,
            vec!["/Users/example/other-project", "/Users/example/project"]
        );

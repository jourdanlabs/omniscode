...
mod tests {
    ...
    fn short_queries_match_late_in_long_tokens() {
        ...
        assert!(fuzzy_score_tokens("opus 5", "claude-opus-4.5 anthropic premium").is_some());
        assert!(fuzzy_score_tokens("opus 5", "claude-opus-4-5-2025 anthropic responses").is_some());
        // Unrelated models still do not match.
        assert!(fuzzy_score_tokens("opus 5", "gpt-5.5 openai responses").is_none());
    }

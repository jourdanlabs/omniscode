...
#[test]
fn test_handle_openai_output_item_recovers_synthetic_wrapped_tool_fixture() {
    let item = serde_json::json!({
        "type": "message",
        "content": [{
            "type": "output_text",
            "text": SYNTHETIC_WRAPPED_TOOL_CALL_FIXTURE,
        }],
    });
    ...
    for event in events {
        match event {
            StreamEvent::TextDelta(text) if text.contains("Synthetic parser fixture prefix") => {
                saw_prefix = true;
            }

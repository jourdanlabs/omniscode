...
//!
//! Cases:
//!   (a) deterministic synthetic 35-item plan
//!   (a23) first 23 items of that synthetic plan
...
fn synthetic_stress_plan() -> Vec<PlanItem> {
    let mut items: Vec<PlanItem> = (0..34)
        .map(|i| {
            let status = if i < 4 {
                "completed"
            } else if i == 4 {
            ...
            item(
                &format!("synthetic-{i:02}"),
                &format!("synthetic task {i:02}"),
                status,
                &[],
            )
        })
    ...
    for i in 1..34 {
        if i % 4 != 0 {
            items[i].blocked_by = vec![format!("synthetic-{:02}", i - 1)];
        }
    }

    let gate_dependencies = (10..25).map(|i| format!("synthetic-{i:02}")).collect();
    items.push(PlanItem {
        content: "synthetic wide fan-in verification".to_string(),
        status: "queued".to_string(),
        priority: "normal".to_string(),
        id: "synthetic::gate".to_string(),
        subsystem: None,
        file_scope: Vec::new(),
        blocked_by: gate_dependencies,
        assigned_to: None,
    });
    items
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // (a) deterministic synthetic plan. Its 35 items exercise truncation and
    // its gate node exercises a wide fan-in without retaining operator data.
    let synthetic = synthetic_stress_plan();
    println!("synthetic fixture items: {}", synthetic.len());
    let src_a = swarm_plan_mermaid(&synthetic).ok_or("graph render returned None")?;
    println!(
        "--- (a) synthetic plan mermaid ({} lines) ---\n{src_a}",
        src_a.lines().count()
    );
    ok &= probe("a-synthetic-plan-full", &src_a);
    let src_a23 = swarm_plan_mermaid(&synthetic[..synthetic.len().min(23)])
        .ok_or("graph render returned None")?;
    ok &= probe("a23-synthetic-plan-23", &src_a23);

...
fn render_swarm_message_centered_mode_caps_wrap_width_for_long_notifications() {
    ...
    let msg = DisplayMessage::swarm(
        "File activity",
        "/home/example/omnis-key/src/tui/ui_messages.rs - moss just edited this file while you were working nearby, so the notification should still read as centered in wide layouts.",
    );
...
fn render_tool_message_shows_discovery_browse_results_and_rationale() {
    let msg = first_discovery_message(
        "Discoverable tools in 'payments' (user-configured third-party catalog; entries are unverified and recommendations must be based only on fit; catalog: https://example.invalid/catalog):\n\n- example-card-tool: sandbox payment test helper (https://example.invalid/card-tool)\n\nBrowse request ID: `11111111-2222-4333-8444-555555555555`",
        serde_json::json!({
            "action": "browse",
            "category": "payments",
    ...
    assert!(plain.contains("example-card-tool"), "{plain}");
    assert!(plain.contains("sandbox payment test helper"), "{plain}");
    assert!(plain.contains("example.invalid/card-tool"), "{plain}");
    assert!(
        plain.contains("This result came from the user-configured discovery endpoint"),
        "{plain}"
    );
    assert!(!plain.contains("partner"), "{plain}");
    assert!(
        lines.len() <= 8,
...
fn batched_discovery_renders_first_use_disclosure_inline_once() {
    ...
    assert_eq!(
        plain
            .matches("This result came from the user-configured discovery endpoint")
            .count(),
        1,
...
fn render_tool_message_shows_selected_discovery_setup() {
    let msg = discovery_message(
        "Selected 'example-card-tool' from 'payments' (user-configured third-party catalog; entry is unverified and selection must be based only on fit; catalog: https://example.invalid/catalog):\n\nexample-card-tool: sandbox payment test helper (https://example.invalid/card-tool)\n\nSetup: Run `example-card-tool --stdio`, then connect the resulting MCP server.\n\nBefore signup, installation, or spending, inspect the provider, terms, privacy, and security with the user. No endorsement, approval, or commercial relationship is implied.",
        serde_json::json!({
            "action": "select",
            "category": "payments",
            "tool": "example-card-tool",
    ...
    let plain = lines
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(plain.contains("selected example-card-tool"), "{plain}");
    assert!(
        plain.contains("details: sandbox payment test helper"),
        "{plain}"
    );
    assert!(
        plain.contains("https://example.invalid/card-tool"),
        "{plain}"
    );
    assert!(plain.contains("setup:"), "{plain}");
    assert!(plain.contains("example-card-tool --stdio"), "{plain}");
    assert!(
        !plain.contains("user-configured discovery endpoint"),
...
fn render_tool_message_shows_catalog_suggestion_receipt_and_trust_line() {
    let msg = discovery_message(
        ...
        serde_json::json!({
            ...
            "product_name": "Example sandbox tool",
            "product_url": "https://example.invalid/sandbox-tool",
            "gap_evidence": "The listed example handles cards rather than sandbox objects.",
            "requirements": [
    ...
    assert!(
        plain.contains("Known product · Example sandbox tool"),
        "{plain}"
    );
    assert!(plain.contains("gap: the listed payment tool"), "{plain}");
    assert!(plain.contains("needs:"), "{plain}");
    assert!(plain.contains("configured catalog endpoint"), "{plain}");
...
fn discovery_cards_wrap_within_narrow_transcript_width() {
    let msg = discovery_message(
        "Catalog suggestion submitted.\n\nStatus: accepted by the configured catalog endpoint.",
        serde_json::json!({
            "action": "suggest",

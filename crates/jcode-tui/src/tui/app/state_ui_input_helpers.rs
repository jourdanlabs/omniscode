...
const REGISTERED_COMMANDS: &[RegisteredCommand] = &[
    ...
    RegisteredCommand::hidden(
        "/remote",
        "Inherited remote pairing is disabled in OMNIS KEY V1",
    ),
    RegisteredCommand::public(
        "/triage",
    ...
    RegisteredCommand::public("/skills", "Show skills already loaded from local files"),
...
fn latest_external_cli_continuation_prompt() -> Option<String> {
    if std::env::var_os("JCODE_DEFERRED_AUTH_BOOTSTRAP").is_some() {
        // Rendering the no-argument integrity shell must not inspect another
        // CLI's session history merely to synthesize a welcome suggestion.
        return None;
    }
...
{"type":"user","message":{"role":"user","content":"Organize my windows by project"},"cwd":"/home/example","sessionId":"abc"}
...
"#,
        ...
        assert_eq!(candidate.working_dir.as_deref(), Some("/home/example"));
        assert_eq!(
    ...
    fn parses_codex_input_text_blocks() {
        ...
        std::fs::write(
            &path,
            r#"{"type":"session_meta","payload":{"id":"sid","cwd":"/home/example/omnis-key"}}
...
"#,
    ...
    #[test]
    fn disabled_remote_pairing_is_not_advertised() {
        assert!(
            !registered_command_entries().any(|(name, _)| name == "/remote"),
            "disabled /remote pairing must not appear in slash help or autocomplete"
        );
    }
}

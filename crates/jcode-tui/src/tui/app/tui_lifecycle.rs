...
impl App {
    ...
    pub(super) fn trip_credential_failure_breaker(&mut self, message: &str) {
        ...
        self.push_display_message(DisplayMessage::error(format!(
            "🛑 Stopped automatic retries: {failures} consecutive credential/auth failures. \
             ...
             request cannot succeed.{} Review locally supplied provider configuration outside \
             OMNIS KEY, or use /model to switch to a working local route, then send again.",
            if cleared_pokes == 0 {
                String::new()
            } else {
    ...
    pub(super) fn new_minimal_with_session(
        ...
        // Remote/minimal startup must not import or inspect ambient MCP
        // configuration. An explicit full local runtime may still construct a
        // normal manager through `App::new`.
        ...
        let mut app = Self {
            ...
            catalog_disclosure_shown_this_session: false,
    ...
    pub fn new(provider: Arc<dyn Provider>, registry: Registry) -> Self {
        ...
        let mcp_manager = if std::env::var_os("JCODE_DEFERRED_AUTH_BOOTSTRAP").is_some() {
            // A no-argument first frame must not inspect inherited global,
            // project, Claude, or Codex MCP configuration.
            Arc::new(RwLock::new(McpManager::with_config(
                crate::mcp::McpConfig::default(),
            )))
        } else {

...
/// Pin JCODE_HOME to a tempdir so resume-command tests cannot consult ambient
/// Jcode state while validating the current-process launch path.
...
fn fresh_session_command_includes_fresh_spawn_and_socket() {
    ...
    assert_eq!(command.title.as_deref(), Some("omnis · new session"));
    assert_eq!(
        command.args,
        vec![
            "--fresh-spawn".to_string(),

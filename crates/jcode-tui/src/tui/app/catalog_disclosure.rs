//! User-configured catalog disclosure placement.
//!
//! The first `discover_tools` result in a session carries a subtle inline
//! boundary notice. Keeping the notice attached to the result makes the
//! unverified third-party source visible without a standalone system message.
...
impl App {
    ...
    pub(in crate::tui::app) fn inline_catalog_disclosure_title(
        &mut self,
        tool: &ToolCall,
    ) -> Option<String> {
        if !should_disclose(self.catalog_disclosure_shown_this_session, tool) {
            return None;
        }
        self.catalog_disclosure_shown_this_session = true;

    ...
    pub fn restore_session(&mut self, session_id: &str) {
        ...
        if let Ok(session) = Session::load(session_id) {
            ...
            if let Some(model) = self.session.model.clone() {
                match crate::provider::MultiProvider::model_switch_request_for_session_route(
                    &model,
                    ...
                    Err(e) => self.push_display_message(DisplayMessage {
                        role: "system".to_string(),
                        content: format!("⚠ Refused persisted model route '{}': {}", model, e),
                        tool_calls: vec![],
                        duration_secs: None,
                        title: None,
                        tool_data: None,
...
pub(super) fn handle_dev_command(app: &mut App, trimmed: &str) -> bool {
    if matches!(
        trimmed,
        "/reload" | "/rebuild" | "/update" | "/client-reload" | "/server-reload"
    ) {

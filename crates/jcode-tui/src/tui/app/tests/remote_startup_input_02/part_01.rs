...
fn test_remote_fallback_claude_model_gets_api_key_route_without_oauth() {
    ...
    with_temp_jcode_home(|| {
        ...
        app.remote_model_options.clear();
        assert!(crate::auth::AuthStatus::check_fast().anthropic.has_api_key);
...
#[test]
fn test_remote_jcode_subscription_catalog_is_quarantined_despite_local_auth() {
    with_temp_jcode_home(|| {
        ...
        assert!(
            ...
            "a hostile or legacy remote must not re-enable inherited subscription routes"
        );
    });
}

#[test]
fn test_remote_mixed_catalog_drops_jcode_subscription_routes() {
...
#[test]
fn test_remote_hydrated_catalog_ignores_local_jcode_subscription_credentials() {
    with_temp_jcode_home(|| {
...
#[test]
fn test_remote_non_jcode_catalog_repairs_poisoned_routes_without_subscription() {
    with_temp_jcode_home(|| {
        let previous_tier = std::env::var_os(crate::subscription_catalog::JCODE_TIER_ENV);
        crate::env::set_var(crate::subscription_catalog::JCODE_TIER_ENV, "plus");

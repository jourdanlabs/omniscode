...
#[test]
fn test_account_openai_command_is_refused_without_opening_picker() {
    with_temp_jcode_home(|| {
        ...
        assert!(app.inline_interactive_state.is_none());
        let message = app.display_messages().last().expect("refusal message");
        assert_eq!(message.role, "error");
        assert!(message.content.contains("inherited Jcode account behavior is disabled"));
    });
}

#[test]
fn test_account_command_is_refused_without_discovering_saved_accounts() {
    with_temp_jcode_home(|| {
        let now_ms = chrono::Utc::now().timestamp_millis();
...
#[test]
fn test_forged_account_picker_enter_is_refused_locally_and_remotely() {
    fn forged_picker() -> crate::tui::InlineInteractiveState {
    ...
    for remote in [false, true] {
        ...
        app.inline_interactive_state = Some(forged_picker());

        app.handle_inline_interactive_key(KeyCode::Enter, KeyModifiers::empty())
            .unwrap();

        assert!(app.inline_interactive_state.is_none());
        assert!(app.pending_account_picker_action.is_none());
        let message = app.display_messages().last().expect("refusal message");
        assert_eq!(message.role, "error");
        assert!(
            message
...
#[test]
fn test_forged_remote_pending_account_action_is_discarded_without_mutation() {
    let mut app = create_test_app();
    let context_limit = app.context_limit;
    app.pending_account_picker_action = Some(crate::tui::AccountPickerAction::Switch {
        provider_id: "openai".to_string(),
        label: "private".to_string(),
    });

    assert!(super::remote::discard_pending_account_picker_action(
        &mut app
    ));
    assert!(app.pending_account_picker_action.is_none());
    assert_eq!(app.context_limit, context_limit);
    let message = app.display_messages().last().expect("refusal message");
    assert_eq!(message.role, "error");
    assert!(
        message
...
#[test]
fn test_account_picker_preview_never_discovers_saved_accounts() {
    with_temp_jcode_home(|| {
        let now_ms = chrono::Utc::now().timestamp_millis();
...
#[test]
fn test_account_command_does_not_combine_or_reveal_saved_accounts() {
    with_temp_jcode_home(|| {
        let now_ms = chrono::Utc::now().timestamp_millis();
        ...
        assert!(!message.content.contains("claude-1"));
        assert!(!message.content.contains("openai-1"));
    });
}
...
fn test_account_command_uses_fast_auth_snapshot_without_running_cursor_status() {
    ...
    with_temp_jcode_home(|| {
        ...
        assert!(
            !marker.exists(),
            "/account refusal must not execute `cursor-agent status`"
        );
        let message = app.display_messages().last().expect("refusal message");
        assert_eq!(message.role, "error");
...
#[test]
fn test_account_switch_shorthand_is_refused_without_switching_account() {
    with_temp_jcode_home(|| {
        let now_ms = chrono::Utc::now().timestamp_millis();
...
#[test]
fn test_login_command_is_refused_without_opening_picker() {
    let mut app = create_test_app();
...
#[test]
fn test_account_openai_compatible_settings_is_refused() {
    let mut app = create_test_app();
...
#[test]
fn test_account_default_provider_command_is_refused_without_config_write() {
    let _guard = crate::storage::lock_test_env();
    let mut app = create_test_app();
    let before = crate::config::Config::load().provider.default_provider;
    app.input = "/account default-provider openai".to_string();
    app.submit_input();

    let after = crate::config::Config::load().provider.default_provider;

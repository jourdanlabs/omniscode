...
#[test]
fn hostile_remote_reloading_event_is_inert_when_updates_are_disabled() {
    let mut app = create_test_app();
    let rt = tokio::runtime::Runtime::new().unwrap();
    let _guard = rt.enter();
    let backend = ratatui::backend::TestBackend::new(90, 20);
    ...
    let mut state = super::remote::RemoteRunState {
        ...
        last_disconnect_reason: Some("server reload in progress".to_string()),
        ..Default::default()
    };
    ...
    let (outcome, needs_redraw) = rt
        .block_on(super::remote::handle_remote_event(
            ...
            &mut remote,
            &mut state,
            crate::tui::backend::RemoteRead::Event(crate::protocol::ServerEvent::Reloading {
                new_socket: Some("/tmp/hostile-reload.sock".to_string()),
            }),
        ))
        .expect("disabled updater should ignore hostile reload event");
    ...
    assert!(!state.reload_recovery_attempted);

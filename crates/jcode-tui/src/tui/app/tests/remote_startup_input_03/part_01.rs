...
#[test]
fn test_v1_update_quarantine_never_reports_a_newer_binary() {
...
#[test]
fn test_v1_update_quarantine_reload_command_has_no_attempt_or_handoff_write() {
    use std::time::SystemTime;
...
#[test]
fn test_hostile_background_update_ready_is_inert() {
    let mut app = create_test_app();
...
#[test]
fn test_pending_background_reload_cannot_bypass_disabled_updater() {
    let mut app = create_test_app();
    let session_id = app.session.id.clone();
    app.pending_background_client_reload =
        Some((session_id, ClientMaintenanceAction::Update));
...
#[test]
fn test_hostile_background_rebuild_status_is_inert() {
    let mut app = create_test_app();
    let session_id = app.session.id.clone();
...
#[test]
fn test_v1_update_quarantine_hostile_startup_progress_is_inert() {
    let mut app = create_test_app();
    let messages_before = app.display_messages().len();
    ...
    assert_eq!(app.display_messages().len(), messages_before);
...
#[test]
fn test_v1_update_quarantine_hostile_divergence_cannot_offer_merge() {
    let mut app = create_test_app();
    let messages_before = app.display_messages().len();
    ...
    assert_eq!(app.display_messages().len(), messages_before);
    assert!(app.pending_merge_offer.is_none());
    assert!(app.background_client_action.is_none());
    assert!(app.pending_background_client_reload.is_none());
    assert!(app.reload_requested.is_none());
    assert!(!app.should_quit);
...
#[test]
fn test_v1_update_quarantine_prefixed_divergence_cannot_arm_merge_offer() {
    let mut app = create_test_app();
    let messages_before = app.display_messages().len();
...
#[test]
fn test_v1_update_quarantine_hostile_error_is_inert() {
    let mut app = create_test_app();
    let messages_before = app.display_messages().len();

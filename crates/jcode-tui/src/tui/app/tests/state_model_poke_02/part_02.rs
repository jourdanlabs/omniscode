...
#[test]
fn test_login_command_suggestions_do_not_enumerate_provider_catalog() {
    ...
    assert!(
        suggestions
            .iter()
            .all(|(command, _)| !command.starts_with("/login")),
        "provider catalog leaked through suggestions: {suggestions:?}"
    );
}

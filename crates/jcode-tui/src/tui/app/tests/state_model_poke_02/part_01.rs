...
#[test]
fn test_registered_command_suggestions_include_retained_aliases_and_hide_quarantined_commands() {
    let app = create_test_app();
    ...
    for inherited in [
        ...
        assert!(
            !commands.contains(&inherited),
            "quarantined command was publicly suggested: {inherited}"
        );
    }
...
#[test]
fn test_top_level_command_suggestions_include_config_but_not_subscription() {
    let app = create_test_app();
    ...
    assert!(!suggestions.iter().any(|(cmd, _)| {
        cmd == "/subscription" || cmd == "/subscribe"
    }));
}
...
#[test]
fn test_top_level_command_suggestions_exclude_quarantined_and_hidden_commands() {
    let app = create_test_app();

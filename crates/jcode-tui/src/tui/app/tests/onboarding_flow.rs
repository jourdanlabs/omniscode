...
#[test]
fn login_openai_phase_is_default_without_ambient_import_discovery() {
    use crate::tui::OnboardingWelcomeKind;
    with_temp_jcode_home(|| {
        let mut app = create_test_app();
        app.onboarding_flow = None;

        // Even an importable fixture must not affect cold onboarding. V1 does
        // not inspect another application's credential store during startup.
        ...
        std::fs::write(
            legacy_auth,
            r#"{"OPENAI_API_KEY":"sk-onboarding-discovery-fixture"}"#,
        )
        .expect("seed importable fixture");
        ...
        assert!(
            ...
            "cold onboarding must not start inherited credential import"
        );
    });
}

#[test]
fn login_openai_no_finishes_onboarding_without_dead_login_hint() {
    with_temp_jcode_home(|| {
        let mut app = create_test_app();
        app.onboarding_flow = None;
        app.begin_onboarding_flow_at_login();
        ...
        // 'n' exits onboarding straight to the normal screen without opening
        // or advertising an inherited account surface.
        ...
        // A system message states the Local Integrity credential boundary.
        ...
        assert!(
            ...
            "guidance message should state the boundary: {:?}",
            messages.last().unwrap().content
        );
        assert!(!messages.last().unwrap().content.contains("/login"));
    });
}
...
#[test]
fn login_phase_enter_refuses_inherited_account_picker_and_esc_exits() {
    with_temp_jcode_home(|| {
        let mut app = create_test_app();
        app.onboarding_flow = None;
        app.begin_onboarding_flow_at_login();
        ...
        // The inherited account picker is quarantined in Local Integrity V1.
        // Enter is consumed with an explicit refusal, without opening or
        // discovering any account/provider state.
        ...
        let messages = app.display_messages();
        assert_eq!(messages.len(), before + 1, "one refusal should be surfaced");
        assert!(
            messages.last().unwrap().content.contains(
                "OMNIS_KEY_INHERITED_SURFACE_DISABLED: inherited Jcode account behavior is disabled"
            ),
            "expected the stable V1 account refusal, got {:?}",
            messages.last().unwrap().content
        );

        // The refusal cannot strand the user: the universal escape hatch still
        // exits onboarding synchronously.
...
#[test]
fn import_failure_resets_login_to_non_discovering_recovery_prompt() {
    use crate::external_auth::ExternalAuthReviewCandidate;
    use crate::tui::app::onboarding_flow::ImportReview;
    ...
    with_temp_jcode_home(|| {
        ...
        // Login phase to a clean recovery prompt so the welcome card stops
        ...
        // Enter must not reopen the inherited account picker. It produces the
        // stable refusal without discovering account state, and Esc exits.
        let before = app.display_messages().len();
        assert!(app.handle_onboarding_continue_prompt_key(KeyCode::Enter));
        assert!(app.inline_interactive_state.is_none());
...
#[test]
fn import_review_decline_all_falls_back_to_non_discovering_recovery() {
    use crate::external_auth::ExternalAuthReviewCandidate;
    use crate::tui::app::onboarding_flow::ImportReview;
    ...
    with_temp_jcode_home(|| {
        ...
        // Still in Login: Enter refuses the inherited picker without opening an
        // account surface. Esc remains a synchronous exit.
        let before = app.display_messages().len();
        assert!(app.handle_onboarding_continue_prompt_key(KeyCode::Enter));
        assert!(app.inline_interactive_state.is_none());
        assert_eq!(app.display_messages().len(), before + 1);
...
#[test]
fn model_validation_auth_failure_avoids_dead_login_guidance() {
    let mut app = create_test_app();
    let session_id = app.session.id.clone();
    ...
    let line = &messages.last().unwrap().content;
    assert!(
        line.contains("outside OMNIS KEY"),
        "auth failure states local boundary: {line:?}"
    );
    assert!(!line.contains("/login"), "dead login guidance leaked: {line:?}");
...
fn remote_post_login_validation_waits_for_catalog_refresh() {
    ...
    with_temp_jcode_home(|| {
        ...
        app.onboarding_pending_model_validation = Some(OnboardingPendingValidation {
            session_id: app.session.id.clone(),
            requested_at: std::time::Instant::now(),
            await_catalog_refresh: true,
            catalog_generation_at_request: 3,
        });
...
fn liveness_every_login_phase_has_a_single_keypress_exit() {
    ...
    with_temp_jcode_home(|| {
        ...
        let cases: Vec<(&str, OnboardingPhase, KeyCode)> = vec![
            ...
            // Recovery fallback: Esc leaves the quarantined compatibility
            // phase without opening inherited account/provider state.
            (
                "Login{import:None}",
...
#[test]
fn liveness_import_review_decline_all_refuses_picker_then_escapes() {
    use crate::external_auth::ExternalAuthReviewCandidate;
    ...
    with_temp_jcode_home(|| {
        ...
        // can't hang) and must land on the recovery screen. Enter must refuse
        // the quarantined inherited picker, and Esc must still leave the flow.
        ...
        // Enter produces a refusal without opening inherited account state.
        let before = app.display_messages().len();
        assert!(app.handle_onboarding_continue_prompt_key(KeyCode::Enter));
        assert!(app.inline_interactive_state.is_none());
        assert_eq!(app.display_messages().len(), before + 1);
        ...
        // Esc is the guaranteed liveness path after the refusal.
        assert!(app.handle_onboarding_continue_prompt_key(KeyCode::Esc));
        assert!(
            onboarding_state_is_escapable(&app),
            "recovery refusal + Esc must leave onboarding"
        );
    });
}
...
fn liveness_stuck_import_is_recovered_by_the_tick_watchdog() {
    ...
    with_temp_jcode_home(|| {
        ...
        // Enter refuses the inherited picker without opening account state;
        // Esc then leaves onboarding, so the watchdog cannot strand the user.
        let before = app.display_messages().len();
        assert!(app.handle_onboarding_continue_prompt_key(KeyCode::Enter));
        assert!(app.inline_interactive_state.is_none());
        assert_eq!(app.display_messages().len(), before + 1);
    ...
    with_temp_jcode_home(|| {
        ...
        runtime.block_on(async {
            ...
            assert_eq!(
                ...
                "the import event should carry the canonical provider family; exact runtime routing is resolved only after this explicit action"
            );
            assert!(

...
#[test]
fn test_remote_catalog_ingestion_quarantines_subscription_identity_and_routes() {
    let mut app = create_test_app();
    let rt = tokio::runtime::Runtime::new().unwrap();
    let _guard = rt.enter();
    let mut remote = crate::tui::backend::RemoteConnection::dummy();
    ...
    app.handle_server_event(
        crate::protocol::ServerEvent::AvailableModelsUpdated {
            provider_name: Some("Jcode Subscription".to_string()),
            provider_model: Some("ordinary-model".to_string()),
            available_models: vec!["ordinary-model".to_string()],
            available_model_routes: vec![
                crate::provider::ModelRoute {
                    model: "ordinary-model".to_string(),
                    provider: "Jcode Subscription".to_string(),
                    api_method: "jcode_subscription".to_string(),
                    available: true,
                    detail: "hostile inherited route".to_string(),
                    cheapness: None,
                },
                crate::provider::ModelRoute {
                    model: "ordinary-model".to_string(),
    ...
    assert_eq!(app.remote_model_options.len(), 1);
    assert_eq!(app.remote_model_options[0].provider, "OpenAI");
    let snapshot = app.remote_model_catalog_snapshot();
    assert!(snapshot.provider_name.is_none());
    assert!(snapshot.model_routes.iter().all(|route| {
        !super::inline_interactive::remote_catalog_value_is_quarantined(&route.provider)
            && !super::inline_interactive::remote_catalog_value_is_quarantined(&route.api_method)
    }));
}
...
#[test]
fn test_remote_model_change_quarantines_hostile_subscription_metadata() {
    let mut app = create_test_app();
    ...
    assert!(app.remote_provider_name.is_none());
    assert!(app.remote_model_options.is_empty());
}

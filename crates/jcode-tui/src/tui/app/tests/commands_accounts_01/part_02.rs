...
fn test_mask_email_censors_local_part() {
    assert_eq!(
        mask_email("sample.user@example.invalid"),
        "s***r@example.invalid"
    );
}

#[test]
fn test_subscription_command_is_disabled_without_advertising_account_access() {
    let _guard = crate::storage::lock_test_env();
    ...
    let msg = app
        .display_messages()
        .last()
        .expect("missing /subscription refusal");
    assert_eq!(msg.role, "error");
    assert!(msg.content.contains("OMNIS_KEY_INHERITED_SURFACE_DISABLED"));
    assert!(!msg.content.contains("/login jcode"));
    assert!(!msg.content.contains("$20/mo"));
}

#[test]
fn test_subscribe_command_is_disabled_without_marketing_pitch() {
    let mut app = create_test_app();
    ...
    let msg = app
        .display_messages()
        .last()
        .expect("missing /subscribe refusal");
    assert_eq!(msg.role, "error");
    assert!(msg.content.contains("OMNIS_KEY_INHERITED_SURFACE_DISABLED"));
    assert!(!msg.content.contains("/login jcode"));
    assert!(!msg.content.contains("$20/mo"));
...
fn test_usage_report_shows_no_connected_providers_when_results_empty() {
    ...
    assert!(msg.content.contains("outside OMNIS KEY Local Integrity V1"));

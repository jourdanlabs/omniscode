...
// Entry-path scoring is restricted to V1-reachable paths; retained compatibility
// phases are still classified and exercised for liveness, but are not presented
// as ambient startup discovery.
//
...
fn entry_paths() -> Vec<Path> {
    vec![
        Path {
            name: "Fresh install, explicitly accept OpenAI sign-in",
            weight: 0.55,
            reaches_ready: true,
            steps: vec![
        ...
        Path {
            name: "Fresh install, decline credential changes",
            weight: 0.15,
            reaches_ready: false,
            steps: vec![
        ...
        Path {
            name: "Cached authenticated state at startup",
            weight: 0.30,
            reaches_ready: true,
            steps: vec![
                Step { phase: "StartChoice", keystrokes: 1, is_decision: true, external_boundary: false },
...
fn node_props(n: GraphNode) -> NodeProps {
    ...
    match n {
        ...
        // Compatibility recovery refuses the inherited account picker; Esc is
        // the guaranteed synchronous exit.
...
fn flow_edges() -> Vec<Edge> {
    use GraphNode::*;
    vec![
        // V1 entry routing from the virtual Start. Cold startup never discovers
        // another application's credentials, so LoginImport has no Start edge.
        ...
        // Retained fixture/compatibility import review: accept/decline each
        // candidate, then suggestions. A failed/declined import drops to the
        // recovery fallback. This component is not ambiently Start-reachable.
        Edge { from: LoginImport, to: StartChoice, keystrokes: 1 },
        Edge { from: LoginImport, to: LoginRecovery, keystrokes: 1 },
        // Recovery: the inherited account picker is refused; Esc leaves the
        // compatibility flow without changing credential state.
...
//
//   * back_navigation       - a declined choice reaches a clean normal surface
//     without advertising or opening a quarantined account command/picker.
//   * error_recovery_depth  - keystrokes from a failed/declined branch back to a
//     clean exit after the inherited picker has been explicitly refused (lower
//     is better).
...
#[derive(Clone, Copy)]
struct Tier8Metrics {
    /// A declined primary choice reaches a clean boundary-respecting exit.
    back_navigation_ok: bool,
    /// Keystrokes from a compatibility recovery branch to a clean exit.
...
fn tier8_metrics() -> Tier8Metrics {
    ...
    // ---- back_navigation: decline OpenAI sign-in ----
    // Declining ('n') finishes onboarding with one boundary message. It must
    // not advertise /login or open inherited account/provider state.
    let back_navigation_ok = {
        ...
        let boundary_message = messages.get(before).is_some_and(|message| {
            message
                .content
                .contains("outside OMNIS KEY Local Integrity V1")
                && !message.content.contains("/login")
        });
        consumed
    ...
    // Recovery depth: from retained Login{import:None}, Enter must explicitly
    // refuse the inherited picker without opening it; Esc must then exit. This
    // two-key worst-case path exercises both sovereignty and liveness.
    let error_recovery_depth = {
        ...
        let before = app.display_messages().len();
        let refused = app.handle_onboarding_continue_prompt_key(KeyCode::Enter)
            && app.inline_interactive_state.is_none()
            && app.login_picker_overlay.is_none()
            && app.account_picker_overlay.is_none()
            && matches!(
            ...
            && app.display_messages().len() == before + 1
            && app
                .display_messages()
                .last()
                .is_some_and(|message| {
        ...
        let exited = app.handle_onboarding_continue_prompt_key(KeyCode::Esc)
            && app.onboarding_phase().is_none();
        if refused && exited {
            2
        } else {
            // The boundary or liveness path failed; report a conservative depth.
            99
        }
    };
...
fn onboarding_eval_scorecard() {
    with_temp_jcode_home(|| {
        ...
        println!("boundary-safe exit     : {}", yn(t8.back_navigation_ok));
        ...
        assert!(t8.back_navigation_ok, "a declined onboarding choice opened or advertised quarantined account state");
...
fn onboarding_eval_graph_fidelity() {
    with_temp_jcode_home(|| {
        ...
        // Every V1 live entry node is reachable from Start via the edges.
        // LoginImport/LoginRecovery and ContinuePrompt are retained
        // compatibility phases with no ambient Start edge.
        let edges = flow_edges();
        for n in [
            GraphNode::LoginOpenAi,
            GraphNode::ModelSelect,
...
enum FeatureClass {
    ...
    Command,
    /// A free-text input / filter field is shown.
    InputField,
...
fn signal_registry() -> Vec<SignalSpec> {
    ...
    vec![
        ...
        SignalSpec { name: "back_navigation", status: Scored, rationale: "Tier8.no_back_nav (a declined choice exits without opening or advertising quarantined account state)", owns_feature: None },
        SignalSpec { name: "error_recovery_depth", status: Scored, rationale: "Tier8.per_recovery_keystroke (refusal plus escape path from a retained recovery phase)", owns_feature: None },
        ...
        SignalSpec { name: "command_affordance", status: Scored, rationale: "owns typed-command screens when present; also Tier3.escape_hatch", owns_feature: Command },
        ...
        SignalSpec { name: "credential_input_field", status: Rejected, rationale: "inherited account/provider credential pickers are disabled in Local Integrity V1 and must not appear in onboarding", owns_feature: InputField },
...
fn all_welcome_screen_texts() -> Vec<(&'static str, String)> {
    ...
    let phases: Vec<(&'static str, OnboardingPhase)> = vec![
        ...
        (
            "ContinuePrompt",
            OnboardingPhase::ContinuePrompt { cli: "Codex", yes_highlighted: true, shown_at: now },
        ),
        ("Suggestions", OnboardingPhase::Suggestions),
    ];
    phases
...
/// Layer C probe surfaces: every welcome screen plus the compatibility
/// recovery refusal. The latter proves Enter does not reconstruct the inherited
/// provider/account picker.
fn all_probe_surface_texts() -> Vec<(&'static str, String)> {
    let mut out = all_welcome_screen_texts();
    out.push(("AccountSurfaceRefusal", render_login_recovery_refusal_text()));
    out
}

/// Drive the real app through the retained recovery action and render the full
/// frame after the inherited account surface has been refused.
fn render_login_recovery_refusal_text() -> String {
    use crossterm::event::KeyCode;
    use ratatui::Terminal;
    use ratatui::backend::TestBackend;
    ...
    // Enter must be consumed without opening the inherited provider picker.
    app.handle_onboarding_continue_prompt_key(KeyCode::Enter);

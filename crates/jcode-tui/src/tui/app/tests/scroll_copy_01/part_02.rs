#[test]
fn test_prompt_jump_ctrl_digit_core_navigation_clamps_to_oldest() {
    ...
    assert_eq!(
        App::ctrl_prompt_rank(&KeyCode::Char('5'), KeyModifiers::CONTROL),
        Some(5)
    );
    ...
    assert!(
        app.status_notice()
            .is_some_and(|notice| notice.starts_with("Ctrl+5: idx=0/1 line=16 max="))
    );
}
...
fn test_scroll_cmd_j_k_fallback_in_app() {
    ...
    assert!(
        ...
        "remote Ctrl+5 was consumed before prompt-rank navigation; status={:?}",
        app.status_notice()
    );
...
#[test]
fn test_remote_prompt_rank_uses_shared_ctrl_digit_mapping_and_scroll_core() {
    let _render_lock = scroll_render_test_lock();
    let (mut app, mut terminal) = create_scroll_test_app(100, 30, 1, 20);
    ...
    let rank = App::ctrl_prompt_rank(&KeyCode::Char('5'), KeyModifiers::CONTROL)
        .expect("Ctrl+5 should map to the fifth most-recent prompt");

...
fn test_open_model_picker_without_routes_shows_actionable_guidance() {
    with_temp_jcode_home(|| {
        ...
        assert!(last.content.contains("outside OMNIS KEY"));
        assert!(!last.content.contains("/login") && !last.content.contains("/account"));
    });
}
...
#[test]
fn test_typing_login_provider_filter_never_opens_inherited_picker() {
    let mut app = create_test_app();
...
#[test]
fn test_login_provider_filter_enter_refuses_without_starting_login() {
    let mut app = create_test_app();
    ...
    assert_eq!(
        ...
        Some(
            "OMNIS_KEY_INHERITED_SURFACE_DISABLED: inherited Jcode account behavior is disabled in OMNIS KEY Local Integrity V1"
        )
    );
}

#[test]
fn test_typing_login_does_not_activate_or_rewrite_inherited_command() {
    let mut app = create_test_app();
...
#[test]
fn test_bare_login_enter_refuses_without_opening_picker() {
    let mut app = create_test_app();
...
#[test]
fn test_login_navigation_cannot_select_or_start_inherited_provider() {
    let mut app = create_test_app();

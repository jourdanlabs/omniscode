...
#[test]
fn connected_remote_pairing_input_refuses_before_remote_send() {
    let mut app = create_test_app();
    app.is_remote = true;
    app.runtime_mode = crate::tui::app::AppRuntimeMode::RemoteClient;
    app.input = "/remote pair".to_string();
    ...
    rt.block_on(super::handle_remote_key(
        &mut app,
        crossterm::event::KeyCode::Enter,
        crossterm::event::KeyModifiers::NONE,
        &mut remote,
    ))
    .expect("disabled pairing command should refuse locally");

    assert!(!app.is_processing, "disabled pairing must not start a turn");
    assert!(
        app.pending_prompt_before_history.is_none(),
        "disabled pairing must not queue a remote prompt"
    );
    assert!(
        app.display_messages()
            .iter()
            .all(|message| message.role != "user"),
        "disabled pairing must not be echoed as a provider-bound user turn"
    );
    assert_eq!(
        app.display_messages()
            .last()
            .expect("pairing refusal")
            .content,
...
fn auth_provider_hint_resolves_every_emitted_login_completed_provider() {
    ...
    //
    // Pairs of (emitted string, expected canonical hint). `None` is correct for
    // auto-import and for inherited login surfaces removed from OMNIS KEY V1.
...
#[test]
fn process_remote_followups_discards_inherited_auto_reload_without_attempting_it() {
    let mut app = create_test_app();
    let rt = tokio::runtime::Runtime::new().expect("runtime");
    let _guard = rt.enter();
    let mut remote = crate::tui::backend::RemoteConnection::dummy();
...
#[test]
fn process_remote_followups_discards_inherited_reload_before_history_loads() {
    let mut app = create_test_app();
    let rt = tokio::runtime::Runtime::new().expect("runtime");
    let _guard = rt.enter();
    let mut remote = crate::tui::backend::RemoteConnection::dummy();
    ...
    assert!(
        ...
        "disabled inherited reload should be consumed even while history is unloaded"
    );
    assert!(!app.auto_server_reload);
    assert_eq!(app.server_auto_reload_attempts, 0);
    assert!(app.display_messages().is_empty());
...
#[test]
fn process_remote_followups_discards_disabled_inherited_reload_silently() {
    let mut app = create_test_app();
    let rt = tokio::runtime::Runtime::new().expect("runtime");
    let _guard = rt.enter();
    let mut remote = crate::tui::backend::RemoteConnection::dummy();
...
#[test]
fn process_remote_followups_never_accumulates_inherited_reload_attempts() {
    let rt = tokio::runtime::Runtime::new().expect("runtime");
    let _guard = rt.enter();
    let mut app = create_test_app();
    app.auto_server_reload = true;

    // Repeated hostile/stale flags remain inert and cannot accumulate attempts.
    for _ in 0..10 {
        let mut remote = crate::tui::backend::RemoteConnection::dummy();
        remote.mark_history_loaded();
        app.pending_server_reload = true;

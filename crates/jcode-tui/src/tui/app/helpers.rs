...
pub(super) fn gather_memory_info(
    ...
    // The credential-free shell renders before any explicit provider or memory
    // action. Its status widget must not instantiate the memory sidecar:
    // backend auto-selection probes OAuth files and API-key environment
    // variables. Keep the first-frame path conservative and empty instead.
...
fn gather_git_info_inner() -> Option<GitInfo> {
    // No-argument startup must not execute PATH-resolved binaries. Repository
    // status is available through the explicit `/git` command.
    None
}

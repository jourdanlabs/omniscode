...
pub(in crate::tui::app) fn remote_catalog_value_is_quarantined(value: &str) -> bool {
    let normalized: String = value
        .chars()
        .filter(|ch| ch.is_ascii_alphanumeric())
        .map(|ch| ch.to_ascii_lowercase())
        .collect();
    normalized == "jcodesubscription"
}

fn remote_catalog_route_is_quarantined(route: &crate::provider::ModelRoute) -> bool {
    remote_catalog_value_is_quarantined(&route.provider)
        || remote_catalog_value_is_quarantined(&route.api_method)
}

fn sanitize_remote_model_catalog_snapshot(
    mut snapshot: jcode_provider_core::ModelCatalogSnapshot,
) -> jcode_provider_core::ModelCatalogSnapshot {
    if snapshot
        .provider_name
        .as_deref()
        .is_some_and(remote_catalog_value_is_quarantined)
    {
        snapshot.provider_name = None;
    }
    snapshot
        .model_routes
        .retain(|route| !remote_catalog_route_is_quarantined(route));
    snapshot
}
...
impl App {
    ...
    pub(super) fn replace_remote_model_catalog_snapshot(
        ...
        let snapshot = sanitize_remote_model_catalog_snapshot(snapshot);
    ...
    fn extend_remote_routes_for_uncovered_models_static(
        ...
        if provider_is_jcode_subscription {
            // A hostile or legacy remote must not re-enable the inherited
            // managed-subscription surface in this fork.
            routes.clear();
            return;
        }
        ...
        routes.retain(|route| {
            !route
                .provider
                .eq_ignore_ascii_case(crate::subscription_catalog::JCODE_PROVIDER_DISPLAY_NAME)
                && !route
                    .api_method
                    .eq_ignore_ascii_case(crate::subscription_catalog::JCODE_ROUTE_API_METHOD)
        });
        if poisoned_by_jcode_subscription {
            // Repair a poisoned legacy cache with ordinary remote fallbacks;
            // never reconstruct inherited subscription routes.
        ...
        let auth = crate::auth::AuthStatus::check_fast_nonblocking();
    ...
    pub(super) fn handle_inline_interactive_key(
        ...
        match code {
            ...
            KeyCode::Enter => {
                ...
                match entry.action {
                    PickerAction::Account(_)
                    | PickerAction::Login(_)
                    | PickerAction::Logout(_)
                    | PickerAction::LogoutAll => {
                        self.inline_interactive_state = None;
                        super::auth::refuse_inherited_auth_or_account_surface(self);
                    }
                    PickerAction::Usage {
                        title,
                        subtitle,

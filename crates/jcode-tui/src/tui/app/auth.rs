...
pub(crate) use self::auth_account_commands::{
    handle_account_command_remote, handle_auth_command, refuse_inherited_auth_or_account_surface,
    save_openai_fast_setting_local,
};
pub(super) use self::auth_types::{PendingAccountInput, PendingLogin};
...
impl App {
    fn refuse_inherited_jcode_subscription(&mut self) {
        crate::subscription_catalog::clear_runtime_env();
        self.push_display_message(DisplayMessage::error(
            crate::subscription_catalog::INHERITED_SUBSCRIPTION_DISABLED_MESSAGE,
        ));
        self.set_status_notice("Inherited subscription disabled");
    }
    ...
    fn record_oauth_preflight(
        ...
        if !notices.is_empty() {
            notices.push(format!(
                "If setup still fails, review the local {} provider configuration outside OMNIS KEY.",
                provider_id
            ));
        }
        notices.join("\n")
    ...
    pub(super) fn show_interactive_login(&mut self) {
        refuse_inherited_auth_or_account_surface(self);
    }
    ...
    pub(super) fn start_login_provider(
        ...
        match provider.target {
            ...
            crate::provider_catalog::LoginProviderTarget::Jcode => {
                self.refuse_inherited_jcode_subscription()
            }
            ...
            PendingLogin::ApiKeyProfile {
                ...
                match save_result {
                    Ok(()) => {
                        ...
                        {
                            format!(
                                "Inherited Jcode account access is disabled in OMNIS KEY Local Integrity V1.\nDocs: {}",
                                docs_url
                            )

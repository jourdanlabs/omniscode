...
impl App {
    ...
    /// unified resolver (curated static tables, then local OpenRouter and
    /// models.dev-format caches) so a metered provider can use known per-model
    /// prices instead of the generic defaults. Honors the active
    ...
    fn refresh_cached_pricing(&mut self, model: &str, is_anthropic: bool, is_openai: bool) {
        ...
        // do NOT memoize the miss. A cache may later be supplied explicitly;
        // retrying is a local read and never schedules network work.

...
fn inherited_surface_for_command(trimmed: &str) -> Option<&'static str> {
    fn classify_topic(topic: &str) -> Option<&'static str> {
        match topic {
            "feedback" | "telemetry" => Some("telemetry"),
            "support" => Some("support"),
            "subscription" | "subscribe" => Some("subscription"),
            "account" | "accounts" => Some("account"),
            "remote" => Some("pairing"),
            "update" | "reload" | "rebuild" | "client-reload" | "server-reload" => Some("updater"),
            "changelog"
            | "fast-release"
            | "remote-release"
            | "cut-release"
            | "commit-push-release" => Some("release"),
            _ => None,
        }
    }

    fn classify_command(command: &str) -> Option<&'static str> {
        // Ordinary prompts such as "update the README" or "remote work is
        // fine" must remain provider-bound text. Claim only an exact,
        // single-slash command token; `//remote` is not a supported spelling.
        let topic = command.strip_prefix('/')?;
...
pub(super) fn handle_inherited_surface_disabled(app: &mut App, trimmed: &str) -> bool {
    let Some(surface) = inherited_surface_for_command(trimmed) else {
        return false;
    };
...
pub(super) fn handle_session_command(app: &mut App, trimmed: &str) -> bool {
    ...
    // After `/remote-release`: the V1 refusal claims only `/remote` plus
    // whitespace/end and never reaches gateway or pairing state.
...
pub(super) fn build_fast_release_prompt() -> String {
    "OMNIS_KEY_INHERITED_SURFACE_DISABLED: inherited Jcode release behavior is disabled in OMNIS KEY Local Integrity V1".to_string()
}

pub(super) fn build_remote_release_prompt() -> String {
    build_fast_release_prompt()
}
...
fn handle_fast_release_command_local(app: &mut App) {
    app.push_display_message(DisplayMessage::error(build_fast_release_prompt()));
}

fn handle_remote_release_command_local(app: &mut App) {
    app.push_display_message(DisplayMessage::error(build_remote_release_prompt()));
}
...
pub(super) fn handle_feedback_command(app: &mut App, trimmed: &str) -> bool {
    ...
    app.push_display_message(DisplayMessage::error(
        "Feedback and telemetry reporting are disabled in OMNIS KEY Local Integrity V1."
            .to_string(),
    ));
    true
}

/// Inherited telemetry controls are retained only as an explicit refusal.
pub(super) fn handle_telemetry_command(app: &mut App, trimmed: &str) -> bool {
    ...
    app.push_display_message(DisplayMessage::error(
        "Telemetry is disabled in OMNIS KEY Local Integrity V1.".to_string(),
    ));
    true
}

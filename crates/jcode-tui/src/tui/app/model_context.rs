...
impl App {
    ...
    fn format_usage_display_card(
        ...
        if refreshing {
            ...
            lines.push(
                "Provider credential management is outside OMNIS KEY Local Integrity V1."
                    .to_string(),
            );
            return lines.join("\n");
        } else {
...
pub(super) fn no_models_available_message(is_remote: bool) -> String {
    let mut lines = vec![
        ...
        "  - Review locally supplied provider configuration outside OMNIS KEY".to_string(),
        "  - Use /model to inspect the available local snapshot".to_string(),
    ];

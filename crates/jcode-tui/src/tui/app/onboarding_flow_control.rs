...
impl App {
    ...
    /// One-shot startup check for a fresh install.
    ///
    ...
    pub(super) fn maybe_begin_onboarding_flow_on_startup(&mut self) {
        ...
        if std::env::var_os("JCODE_DEFERRED_AUTH_BOOTSTRAP").is_some() {
            // The integrity shell deliberately renders a usable local first
            // frame before any inherited account, setup-history, session, or
            // external-client discovery. An explicit operator action may
            // enter onboarding later.
        ...
        // Startup rendering is a cached-only boundary: never probe credential
        // files here. A snapshot populated by an explicit auth action may
        // continue post-login onboarding; a cold process conservatively lands
        // at the Login phase.
        self.onboarding_startup_checked = true;
        if crate::auth::AuthStatus::check_fast_nonblocking().has_any_available() {
            self.begin_onboarding_flow();
        } else {
    ...
    pub(super) fn begin_onboarding_flow_at_login(&mut self) {
        ...
        // Cold startup never enumerates another application's credential
        // stores. Import review is reserved for an explicit login/review action.
        self.onboarding_flow = Some(OnboardingFlow::begin_at_login(None));
    ...
    /// Start the explicitly selected first-run OpenAI login.
    pub(super) fn onboarding_start_default_login(&mut self) {
        ...
        self.set_status_notice("Login: opening explicitly selected OpenAI sign-in");
    }
    ...
    pub(super) fn handle_onboarding_continue_prompt_key(&mut self, code: KeyCode) -> bool {
            ...
            )
        {
            ...
            self.set_status_notice("Onboarding skipped");
            return true;
        }
    ...
    /// Answer the "Log in to OpenAI?" prompt. Yes starts the explicitly
    /// selected OpenAI sign-in; No exits onboarding without advertising the
    /// quarantined inherited account commands.
    pub(super) fn onboarding_answer_login_openai(&mut self, wants_openai: bool) {
        ...
        if wants_openai {
            ...
            self.push_display_message(DisplayMessage::system(
                "Onboarding finished without changing provider credentials. Credential management is outside OMNIS KEY Local Integrity V1."
                    .to_string(),
            ));
            self.set_status_notice("Onboarding complete");
        }
    }
    ...
    /// intentionally keep it clean: the usual login/import system chatter and
    /// inherited live model validation are both suppressed. No-argument startup
    /// must remain local-only until the user explicitly requests a provider
    /// operation or submits a turn.
    ...
    /// Local Integrity onboarding never enumerates other provider accounts.
    fn onboarding_other_provider_rows(_skip: Option<&str>) -> (Vec<String>, Vec<String>) {
        (Vec::new(), Vec::new())
    }
    ...
    pub(super) fn handle_onboarding_model_validated(
        ...
        if !attention.is_empty() {
            ...
            let fix_hint = if looks_like_auth || !ready.is_empty() {
                "Review local provider configuration outside OMNIS KEY, or use /model to pick another."
                    .to_string()
            } else {
                "Use /model to pick another available local route.".to_string()
            };
            body.push_str(&format!("\n{fix_hint}"));
        }
        if body.is_empty() {
        ...
        if attention.is_empty() {
            ...
            let hint = if looks_like_auth {
                "review local provider configuration outside OMNIS KEY, or /model".to_string()
            } else {

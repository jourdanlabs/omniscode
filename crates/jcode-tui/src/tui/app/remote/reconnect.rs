...
fn should_reload_client(server_reload_in_progress: bool, has_newer_binary: bool) -> bool {
    crate::update::updates_enabled() && (server_reload_in_progress || has_newer_binary)
}
...
pub(in crate::tui::app) async fn handle_post_connect<B: ratatui::backend::Backend>(
    ...
    if state.reconnect_attempts > 0 {
        ...
        let must_reload_client =
            should_reload_client(state.server_reload_in_progress, app.has_newer_binary());
...
mod updater_policy_tests {
    use super::should_reload_client;

    #[test]
    fn hostile_reload_flags_cannot_bypass_disabled_updater_policy() {
        assert!(!should_reload_client(true, false));
        assert!(!should_reload_client(false, true));
        assert!(!should_reload_client(true, true));
    }
}

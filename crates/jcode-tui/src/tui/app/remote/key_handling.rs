    ...
    match code {
        ...
        KeyCode::Enter => {
            ...
            if !app.input.is_empty() {
                ...
                if app_mod::commands::handle_inherited_surface_disabled(app, trimmed) {
                    return Ok(());
                }
                ...
                if matches!(
                    trimmed,
                    "/fast-release" | "/remote-release" | "/cut-release" | "/commit-push-release"
                ) {
                    app.push_display_message(DisplayMessage::error(
                        "OMNIS_KEY_INHERITED_SURFACE_DISABLED: inherited Jcode release behavior is disabled in OMNIS KEY Local Integrity V1",
                    ));
                    return Ok(());
                }

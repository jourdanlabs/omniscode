...
fn rate_limit_notice(reset_secs: u64) -> String {
    format!(
...
fn should_defer_history_for_runtime_identity(
    _server_has_update: Option<bool>,
    ...
    let client_detected_stale =
        server_release_is_older_than_client(_server_version, &client_release_version());
...
#[cfg(test)]
#[path = "server_events_runtime_identity_tests.rs"]
    ...
    match event {
        ...
        ServerEvent::Error {
            ...
            if let Some(reset_duration) = reset_duration {
                ...
                {
                    let rate_limit_line = rate_limit_notice(reset_duration.as_secs());
        ...
        ServerEvent::History {
            ...
            app.remote_server_short_name = server_name.clone();
            app.remote_server_icon = server_icon.clone();
            app.remote_server_has_update = if crate::update::updates_enabled() {
                server_has_update
            } else {
                None
            };
        ...
        ServerEvent::ModelChanged {
            ...
            if let Some(err) = error {
                ...
                app.clear_remote_startup_phase();
                if provider_name.as_deref().is_some_and(
                    crate::tui::app::inline_interactive::remote_catalog_value_is_quarantined,
                ) {
                    app.remote_provider_name = None;
                    app.remote_model_options.retain(|route| {
                        !crate::tui::app::inline_interactive::remote_catalog_value_is_quarantined(
                            &route.provider,
                        ) && !crate::tui::app::inline_interactive::remote_catalog_value_is_quarantined(
                            &route.api_method,

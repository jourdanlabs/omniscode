...
use super::inherited_surface_for_command;
use super::parse_diff_mode_name;
use super::parse_manual_subagent_spec;

#[test]
fn inherited_pairing_surface_uses_exact_remote_command_boundaries() {
    for command in [
        "/remote",
        "/remote pair",
        "/remote\tstatus",
        ...
        assert_eq!(
            inherited_surface_for_command(command),
            Some("pairing"),
            "{command}"
        );
    }
    ...
    assert_eq!(
        inherited_surface_for_command("/remote-release"),
        Some("release")
    );
    for unrelated in ["/remote-skill", "/remotely", "/remot"] {
        assert_ne!(
            inherited_surface_for_command(unrelated),
            Some("pairing"),
            "{unrelated}"
        );
    }
...
#[test]
fn inherited_surface_classifier_never_claims_plain_agent_prompts() {
    for prompt in [
        "update the README",
        "support this platform",
        "account for edge cases",
        "feedback on this diff",
        "remote work is fine",
        "reload the fixture",
        "//remote pair",
        "//update now",
    ] {
        assert_eq!(inherited_surface_for_command(prompt), None, "{prompt}");
    }
    ...
    for command in [
        ...
        assert!(
            inherited_surface_for_command(command).is_some(),
            "{command}"
        );
    }
}

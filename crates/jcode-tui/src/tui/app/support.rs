...
#[cfg(test)]
pub(super) const SUPPORT_EMAIL: &str = "disabled@invalid";
#[cfg(test)]
pub(super) const SUPPORT_SUBJECT: &str = "inherited support disabled";
...
pub(super) fn handle_support_command(app: &mut App, trimmed: &str) -> bool {
    ...
    app.push_display_message(DisplayMessage::error(
        "OMNIS_KEY_INHERITED_SURFACE_DISABLED: inherited Jcode support behavior is disabled in OMNIS KEY Local Integrity V1".to_string(),
    ));
    app.set_status_notice("Inherited support is disabled");
    true
}
...
mod tests {
    ...
    fn support_mailto_url_is_correctly_encoded() {
        ...
        assert!(
            url.starts_with("mailto:disabled@invalid?subject=inherited%20support%20disabled&body=")
        );

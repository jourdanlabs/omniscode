...
pub(super) fn discard_pending_account_picker_action(app: &mut App) -> bool {
    if app.pending_account_picker_action.take().is_none() {
        return false;
    }
    super::auth::refuse_inherited_auth_or_account_surface(app);
    true
}
    ...
    match event {
        ...
        Some(Ok(Event::Key(key))) => {
            ...
            if matches!(key.kind, KeyEventKind::Press | KeyEventKind::Repeat) {
                ...
                discard_pending_account_picker_action(app);
            }
            needs_redraw = true;
            needs_redraw |= dispatch_compacted_history_load(app, remote).await;
        }
    ...
    match event {
        ...
        RemoteRead::Event(ServerEvent::Reloading { new_socket }) => {
            ...
            if !crate::update::updates_enabled() {
                ...
                if state.last_disconnect_reason.as_deref() == Some("server reload in progress") {
...
async fn dispatch_pending_server_reload(app: &mut App, remote: &mut RemoteConnection) {
    if !crate::update::updates_enabled() {
        ...
        app.remote_server_has_update = None;
...
pub(super) async fn process_remote_followups(app: &mut App, remote: &mut RemoteConnection) {
    ...
    if app.pending_background_client_reload.is_some() && !app.is_processing {
        if crate::update::updates_enabled() {
            ...
            app.pending_background_client_reload = None;
        }
        return;
    }

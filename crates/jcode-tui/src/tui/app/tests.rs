...
include!("tests/inherited_account_surface.rs");
include!("tests/inherited_release_surface.rs");
...
include!("tests/remote_events_reload_v1_quarantine.rs");
...
include!("tests/info_widget_cached_auth.rs");
...
#[test]
fn version_command_shows_identity_without_update_or_channel_advertising() {
    let mut app = create_test_app();
    app.is_remote = true;
    ...
    assert!(!content.contains("installed"), "{content}");
    assert!(!content.contains("channel"), "{content}");
}

#[test]
fn changelog_command_is_unconditionally_quarantined() {
    let mut app = create_test_app();
    assert!(super::state_ui::handle_info_command(&mut app, "/changelog"));
    assert!(app.changelog_scroll.is_none());
    let content = app.display_messages().last().unwrap().content.clone();
    assert!(
        content.contains("OMNIS_KEY_INHERITED_SURFACE_DISABLED"),
        "{content}"
    );
    assert!(
        content.contains("release behavior is disabled"),
        "{content}"
    );
}

#[test]
fn skills_command_lists_only_loaded_skills() {
    let mut app = create_test_app();
    ...
    assert!(!content.contains("Endorsed skills"), "{content}");
    assert!(!content.contains("[not installed]"), "{content}");
    assert!(!content.contains("NVIDIA CUDA-X"), "{content}");
    assert!(!content.contains("npx skills add"), "{content}");
    assert!(!content.contains("github.com/NVIDIA/skills"), "{content}");
    assert_eq!(
...
#[test]
fn missing_endorsed_skill_invocation_uses_uniform_unknown_error() {
    let mut app = create_test_app();
    let temp = tempfile::tempdir().expect("tempdir");
    app.session.working_dir = Some(temp.path().to_string_lossy().to_string());
    ...
    assert_eq!(
        last.content,
        format!("Unknown skill: /{}", endorsed.name),
        "missing skills must never surface catalog or install behavior"
    );
}

#[test]
fn stale_server_update_metadata_is_ignored_while_history_applies() {
    ...
    assert_eq!(app.remote_server_has_update, None);
    assert_eq!(app.remote_server_version.as_deref(), Some("v0.0.1-stale"));
    assert_eq!(
        app.remote_session_id.as_deref(),
        Some("session_from_stale_server")
    );
    assert_eq!(remote.session_id(), Some("session_from_stale_server"));
    assert_eq!(app.connection_type.as_deref(), Some("stale-connection"));
    assert_eq!(app.remote_skills, vec!["stale-skill".to_string()]);
    assert_eq!(
        app.session.subagent_model.as_deref(),
        Some("stale-subagent")
    );
    assert_eq!(app.server_auto_reload_attempts, 0);
}

#[test]
fn disabled_version_check_applies_history_without_reload_handoff() {
    ...
    assert!(app.pending_reload_session_id.is_none());
    assert_eq!(remote.session_id(), Some("session_real_server_owned"));
    assert_eq!(app.server_auto_reload_attempts, 0);
...
#[test]
fn ancient_server_history_cannot_activate_client_side_release_behavior() {
    let _env_guard = crate::storage::lock_test_env();
    crate::env::remove_var("JCODE_ALLOW_SERVER_VERSION_MISMATCH");
    ...
    assert_eq!(
        app.remote_session_id.as_deref(),
        Some("session_from_ancient_server")
    );
    assert_eq!(remote.session_id(), Some("session_from_ancient_server"));
    assert_eq!(app.remote_skills, vec!["ancient-skill".to_string()]);
    assert_eq!(
        app.session.subagent_model.as_deref(),
...
#[test]
fn older_server_release_metadata_cannot_schedule_a_reload() {
    let _env_guard = crate::storage::lock_test_env();
    crate::env::remove_var("JCODE_ALLOW_SERVER_VERSION_MISMATCH");
    ...
    assert_eq!(
        app.remote_session_id.as_deref(),
        Some("session_from_old_server")
    );
    assert_eq!(remote.session_id(), Some("session_from_old_server"));
    assert_eq!(app.server_auto_reload_attempts, 0);
}

#[test]
fn older_server_history_does_not_repair_or_advance_shared_server_channel() {
    // A hostile inherited release layout must remain unchanged while V1 handles
    // the remote history normally.
    ...
    let shared_server = crate::build::read_shared_server_version().ok().flatten();
    let pending = app.pending_server_reload;
    let attempts = app.server_auto_reload_attempts;
    ...
    assert!(
        !pending,
        "inherited release metadata must not queue a reload"
    );
    assert_eq!(attempts, 0);
    assert_eq!(
        shared_server.as_deref(),
        Some(old),
        "history handling must not repair or advance inherited release channels"
    );
}

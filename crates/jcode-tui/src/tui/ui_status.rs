...
pub(super) fn is_running_stable_release() -> bool {
    if std::env::var_os("JCODE_DEFERRED_AUTH_BOOTSTRAP").is_some() {
        // Stable-channel detection follows symlinks under inherited build
        // directories. The no-argument integrity shell must render without
        // touching any updater or replacement channel.
        return false;
    }

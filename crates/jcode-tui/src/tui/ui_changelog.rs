...
pub(super) fn get_unseen_changelog_entries() -> &'static Vec<String> {
    ...
    // Inherited release discovery is quarantined in OMNIS KEY V1. Rendering a
    // first frame must not consult or mutate a per-user "last seen release"
    // marker or expose the inherited release surface.

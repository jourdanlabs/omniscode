        ...
        OnboardingWelcomeKind::Login {
            ...
            match import {
                ...
                Some(prompt) if !prompt.choosing => {
                    ...
                    // "Import less" opens the per-login list.
...
pub(super) fn draw_onboarding_welcome(frame: &mut Frame, app: &dyn TuiState, area: Rect) {
    ...
    let donut_h = DONUT_HEIGHT.min(
        area.height
            .saturating_sub(TITLE_H + HINT_H + body_h + GAP + 1),
    );
    ...
    // chunks[0] = top pad, then optional title/donut/hint/gap, then body.
    let mut idx = 1;
    if show_donut_block {
        frame.render_widget(

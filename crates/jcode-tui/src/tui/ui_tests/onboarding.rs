...
fn onboarding_state() -> TestState {
    TestState {
        ...
        suggestions: vec![
            ("Review integrity help".to_string(), "/help".to_string()),
            (
...
#[test]
fn onboarding_welcome_shows_title_and_suggestions_without_telemetry_claims() {
    let state = onboarding_state();
    let text = render_onboarding(&state, 80, 30);

    assert!(
        !text.to_ascii_lowercase().contains("telemetry"),
        "telemetry controls must not be rendered:\n{text}"
    );
    assert!(
        !text.contains("anonymous usage statistics"),
        "telemetry collection claims must not be rendered:\n{text}"
    );
    assert!(
    ...
    assert!(
        text.contains("Review integrity help"),
        "integrity suggestion should be rendered:\n{text}"
    );
    assert!(
...
#[test]
fn onboarding_welcome_integrity_suggestion_shows_typed_command() {
    let state = onboarding_state();
    let text = render_onboarding(&state, 80, 30);
    assert!(
        text.contains("(type /help)"),
        "integrity suggestion should hint the slash command:\n{text}"
    );
    assert!(
        !text.contains("/login"),
        "dead login command leaked:\n{text}"
    );
}
...
fn onboarding_welcome_centers_within_tall_area() {
    // A tall area should leave blank padding above the welcome content.
    let state = onboarding_state();

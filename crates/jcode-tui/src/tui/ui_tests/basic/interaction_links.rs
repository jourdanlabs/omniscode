...
fn test_prompt_entry_shimmer_color_moves_across_positions() {
    let base = user_text();
    // Sample each frame at its moving highlight and well outside it. At the
    // tail of the animation the intentionally faint shimmer can quantize back
    // to the base color on 256-color terminals, so use two frames where the
    // highlight is still visibly distinct in both supported color modes.
    let left_early = prompt_entry_shimmer_color(base, 0.0, 0.0);
    let middle_early = prompt_entry_shimmer_color(base, 0.23, 0.0);
    let left_middle = prompt_entry_shimmer_color(base, 0.0, 0.2);
    let middle_middle = prompt_entry_shimmer_color(base, 0.23, 0.2);

    assert_ne!(left_early, middle_early);
    assert_ne!(left_middle, middle_middle);
    assert_ne!(left_early, left_middle);
}

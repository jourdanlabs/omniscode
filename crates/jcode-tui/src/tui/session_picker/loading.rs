...
pub fn load_sessions() -> Result<Vec<SessionInfo>> {
    if std::env::var_os("JCODE_DEFERRED_AUTH_BOOTSTRAP").is_some() {
        // The no-argument integrity shell does not pre-index native history or
        // discover transcripts owned by Claude, Codex, Pi, OpenCode, or
        // Cursor. Session discovery begins only in a non-deferred runtime.
        return Ok(Vec::new());
    }
    ...
    let candidates = if sessions_dir.exists() {
        // Gather a cheap recency window; the summary pass filters empty sessions.
        let mut candidates =
    ...
    // Load catch-up "seen" state once, not per candidate.
    ...
    let (mut sessions, external_sessions) = std::thread::scope(|scope| {
        ...
        // Fill in parallel windows; beyond `boundary`, retain only saved sessions.
        // At most the final window over-parses.
        let mut sessions: Vec<SessionInfo> = Vec::new();
        // Keep a separate debug budget so workers cannot crowd out normal sessions.
...
pub(crate) fn load_external_cli_sessions_grouped(
    ...
    let sessions = match cli {
        "Codex" => load_external_codex_sessions(scan_limit),
        "Claude Code" => load_external_claude_code_sessions(scan_limit),
        "Pi" => load_external_pi_sessions(scan_limit),
        "OpenCode" => load_external_opencode_sessions(scan_limit),
        "Cursor" => load_external_cursor_sessions(scan_limit),
        _ => Vec::new(),
    };
    (Vec::new(), sessions)
}

...
impl PermissionsApp {
    ...
    fn approve_selected(&mut self) -> Result<()> {
        if let Some(req) = self.requests.get(self.selected) {
            let id = req.id.clone();
            safety::record_permission_via_file(&id, true, "permissions_tui", None)?;
    ...
    fn deny_selected(&mut self, reason: Option<String>) -> Result<()> {
        if let Some(req) = self.requests.get(self.selected) {
            let id = req.id.clone();
            safety::record_permission_via_file(&id, false, "permissions_tui", reason)?;
            self.requests.remove(self.selected);
            self.denied_count += 1;
            if self.selected >= self.requests.len() && self.selected > 0 {
                self.selected -= 1;
    ...
    fn deny_all(&mut self) -> Result<()> {
        while !self.requests.is_empty() {
            let id = self.requests[0].id.clone();
            safety::record_permission_via_file(&id, false, "permissions_tui", None)?;
            self.requests.remove(0);
            self.denied_count += 1;
        }
        self.selected = 0;
        ...
        let result = loop {
                ...
                && let Event::Key(key) = event::read()?
            {
                ...
                if let Some(ref mut text) = self.deny_input {
                    match key.code {
                        KeyCode::Enter => {
                            ...
                            if let Err(error) = self.deny_selected(reason) {
                                break Err(error);
                            }
                        }
                        KeyCode::Esc => {
                ...
                match key.code {
                    ...
                    KeyCode::Char('a') => {
                        if let Err(error) = self.approve_selected() {
                            break Err(error);
                        }
                    }
                    KeyCode::Char('d') => {
                    ...
                    KeyCode::Char('A') => {
                        if let Err(error) = self.approve_all() {
                            break Err(error);
                        }
                    }
                    KeyCode::Char('D') => {
                        if let Err(error) = self.deny_all() {
                            break Err(error);
                        }
                    }
                    _ => {}
...
pub fn run_permissions() -> Result<()> {
    ...
    let reconciled = safety::flush_receipt_outbox()?;
    ...
    if requests.is_empty() {
        if reconciled > 0 {
            println!(
                "Reconciled {} pending safety receipt{}.",
                reconciled,

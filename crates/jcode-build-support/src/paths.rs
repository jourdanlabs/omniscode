...
pub fn selfdev_build_command_for_target(
    repo_dir: &Path,
    _target: SelfDevBuildTarget,
) -> SelfDevBuildCommand {
    let specs = [("jcode", "jcode")];
...
mod tests {
    ...
    /// Every retained build-target spelling resolves to the sole shipped
    /// compatibility binary.
    #[test]
    fn every_build_target_builds_only_jcode() {
        let repo = repo_fixture(false);
        for target in [
            SelfDevBuildTarget::Auto,
            SelfDevBuildTarget::Tui,
            SelfDevBuildTarget::All,
        ] {
            let command = selfdev_build_command_for_target(repo.path(), target);
            assert_eq!(
                command.display, "cargo build --profile selfdev -p jcode --bin jcode",
                "{target:?} exposed a non-V1 build target"
            );
        }
    }
    ...
    fn build_targets_parse_from_their_names() {
        for (name, expected) in [
            ...
            ("jcode", SelfDevBuildTarget::Tui),
            ("all", SelfDevBuildTarget::All),
            ("both", SelfDevBuildTarget::All),

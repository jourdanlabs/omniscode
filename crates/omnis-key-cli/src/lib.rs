...
enum TopLevel {
    ...
    /// Claim Ledger — ground or refuse agent claims about a codebase.
    Claims {
        #[command(subcommand)]
...
#[derive(Debug, Subcommand)]
enum ClaimsCommand {
    /// Show the claim ledger chain status.
    Status {
        #[arg(long)]
        json: bool,
    },
    /// Verify one or more structured claims with deterministic verifiers.
    Verify {
        /// Comma-separated paths claimed as the exclusive change set.
        #[arg(long, value_name = "PATHS")]
        files_changed: Option<String>,
        /// Git base ref for files-changed (default: HEAD).
        #[arg(long)]
        base: Option<String>,
        /// Build command as comma-separated argv, e.g. `cargo,check`.
        #[arg(long, value_name = "CSV_ARGV")]
        build_command: Option<String>,
        /// Test command as comma-separated argv, e.g. `cargo,test,--lib`.
        #[arg(long, value_name = "CSV_ARGV")]
        test_command: Option<String>,
        /// Include demo refusal claims (performance / idiomatic) — pitch surface.
        #[arg(long)]
        refuse_demo: bool,
        /// Raw ClaimInput or array JSON.
        #[arg(long, value_name = "JSON")]
        claim_json: Option<String>,
        /// Repository root (default: cwd).
        #[arg(long, value_name = "DIR")]
        repo: Option<std::path::PathBuf>,
        /// Claim ledger path (default: ~/.jcode/state/omnis-key/claim-ledger.jsonl).
        #[arg(long, value_name = "PATH")]
        ledger: Option<std::path::PathBuf>,
        /// Exit 2 if any claim is REFUTED (CI hook).
        #[arg(long)]
        claims_strict: bool,
        #[arg(long)]
        json: bool,
...
fn dispatch(command: TopLevel, stdout: &mut dyn Write, stderr: &mut dyn Write) -> i32 {
    match command {
        ...
        TopLevel::Claims { action } => run_claims(action, stdout, stderr),
        TopLevel::Omnis { action } => run_compatibility(action, stdout, stderr),
    }
}

fn run_claims(action: ClaimsCommand, stdout: &mut dyn Write, stderr: &mut dyn Write) -> i32 {
    match action {
        ClaimsCommand::Status { json } => claims::status(json, stdout, stderr),
        ClaimsCommand::Verify {
            ...
            claims::VerifyArgs {
                files_changed,
                base,
                build_command: build_command.as_deref().map(claims::parse_csv_command),
                test_command: test_command.as_deref().map(claims::parse_csv_command),
                refuse_demo,
                claim_json,
                repo: repo.unwrap_or_else(|| {

...
const BROWSER_BRIDGE_RELEASE_ENDPOINT: &str = "disabled://omnis-key-v1";
const BROWSER_INTEGRATION_DISABLED_MESSAGE: &str = "OMNIS_KEY_INHERITED_BROWSER_DISABLED: inherited browser bridge sessions, status, readiness, and setup are disabled in OMNIS KEY Local Integrity V1";

fn inherited_browser_integration_disabled() -> bool {
    true
}

const NATIVE_HOST_NAME: &str = "firefox_agent_bridge";
const EXTENSION_ID_DISABLED: &str = "browser-integration-disabled@omnis-key.invalid";
...
pub fn ensure_browser_session(session_id: &str) -> Option<String> {
    if inherited_browser_integration_disabled() {
        return None;
    }
...
pub async fn ensure_browser_setup() -> Result<String> {
    if inherited_browser_integration_disabled() {
        anyhow::bail!(BROWSER_INTEGRATION_DISABLED_MESSAGE);
    }
...
async fn download_browser_binary() -> Result<()> {
    ...
    let mut request = client
        .get(BROWSER_BRIDGE_RELEASE_ENDPOINT)

...
/// Inert compatibility hook. Local Integrity V1 never meters MCP calls.
...
impl McpManager {
    ...
    pub async fn connect(&self, name: &str, config: &McpServerConfig) -> Result<()> {
        // Compatibility hook is intentionally inert in Local Integrity V1.
        if let Some(sponsor) =
            crate::sponsors::provenance::on_server_connected(name, &config.command, &config.args)
        {
            crate::logging::info(&format!(
                "MCP: '{name}' connected with catalog attribution: {sponsor}"
            ));
        }
        if config.shared {
            if let Some(pool) = &self.pool {
    ...
    pub async fn disconnect_all(&self) {
        // Compatibility hook is inert; shutdown never reports MCP activity.
...
"##;
    ...
    /// Discovery history must not tag or meter a real MCP connection.
    #[tokio::test]
    ...
    async fn discovery_attribution_is_inert_with_real_mcp_server() {
        ...
        // 3. Real tool calls through the manager remain ordinary calls.
        let result = manager
        ...
        // 5. No aggregate or report is produced.

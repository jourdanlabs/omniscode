...
fn test_skill_prompt_integration() {
    ...
    assert!(
        prompt.contains("Your name is Jcode, the inherited compatibility chassis for OMNIS KEY.")
    );
...
fn test_selfdev_prompt_uses_full_selfdev_instructions() {
    ...
    assert!(prompt.contains("You are working on the jcode codebase itself."));
    assert!(prompt.contains("targets the TUI/root jcode compatibility chassis"));
...
fn test_split_selfdev_prompt_defaults_to_tui_focus_for_repo_root() {
    ...
    assert!(
        split
            .static_part
            .contains("targets the TUI/root jcode compatibility chassis")
    );

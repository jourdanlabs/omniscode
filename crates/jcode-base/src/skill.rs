...
/// Maximum bounded depth for varied Claude Code plugin skill layouts.
const PLUGIN_SCAN_MAX_DEPTH: usize = 5;

impl SkillRegistry {
    /// Process-wide mutable GLOBAL registry for skill tools and slash paths.
    /// Project-local skills remain per-session overlays and never enter it.
    ...
    /// Load an immutable snapshot of global skills.
    pub fn shared_snapshot() -> Arc<Self> {
        if std::env::var_os("JCODE_DEFERRED_AUTH_BOOTSTRAP").is_some() {
            // The no-argument shell must not scan any skill location.
    ...
    /// Load shared plugin, jcode, and agents skills; exclude project overlays.
    ...
    /// Load a fresh jcode/agents/Claude skill overlay for one workspace.
    ...
    /// Merge a project overlay, which wins same-named global entries.
    pub fn merge_overlay(&mut self, overlay: Self) {
        self.skills.extend(overlay.skills);
    }

    /// Compose global skills with one session's project overlay.
    ...
    /// Load all skills for an optional active-session working directory.
    ...
    /// Reload shared global sources only; never leak a project overlay.

...
///
/// This heuristic reduces fixed top-k padding without adding a second model
/// call. It cannot drop to zero for a non-empty candidate list; sidecar mode
/// remains the path that may omit every candidate.
...
impl MemoryAgent {
    ...
    async fn process_context(
        ...
        // irrelevant ones; we surface the top MAX_MEMORIES_PER_TURN. It uses one
        // listwise call instead of per-candidate binary checks.
...
/// Final extraction is quarantined in OMNIS KEY Local Integrity V1.
...
pub fn trigger_final_extraction_with_dir(
    ...
    // No background provider call, transcript export, or memory write.
}

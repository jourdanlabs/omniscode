...
impl fmt::Display for AccountApiError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Disabled => write!(
                f,
                "OMNIS_KEY_INHERITED_ACCOUNT_DISABLED: inherited Jcode account API is disabled in OMNIS KEY Local Integrity V1"
            ),
            ...
            Self::LegacyBackend => write!(
                f,
                "the configured Jcode API uses an unsupported legacy login contract"
            ),
...
pub async fn request_device_authorization(
    ...
    _api_base: &str,
    _requested_tier: Option<JcodeTier>,
) -> std::result::Result<DeviceAuthorization, AccountApiError> {
    Err(AccountApiError::Disabled)
}

#[cfg(test)]
async fn request_device_authorization_impl(
...
#[cfg(test)]
async fn poll_device_token_once_impl(
    client: &reqwest::Client,
    api_base: &str,
    device_code: &str,
) -> std::result::Result<TokenPollOutcome, AccountApiError> {
...
#[cfg(test)]
async fn fetch_subscription_me_with_impl(
    client: &reqwest::Client,
    api_base: &str,
    api_key: &str,
) -> std::result::Result<SubscriptionMe, AccountApiError> {
...
pub async fn fetch_subscription_me() -> Result<SubscriptionMe> {
    Err(AccountApiError::Disabled.into())
}
...
#[cfg(test)]
async fn revoke_current_key_impl(
    client: &reqwest::Client,
    api_base: &str,
    api_key: &str,
) -> std::result::Result<(), AccountApiError> {
...
mod tests {
    ...
    fn subscription_me_parses_expected_shape() {
        let json = r#"{
            ...
            "manage_url": "https://example.invalid/account"
        }"#;
        ...
        assert_eq!(
            ...
            Some("https://example.invalid/account")
        );
    }
    ...
    async fn device_request_uses_new_contract_and_parses_public_urls() {
        let base = spawn_server(vec![(
            200,
            vec![],
            r#"{"device_code":"secret","flow_id":"public-flow","verification_uri":"https://example.invalid/account","verification_uri_complete":"https://example.invalid/account?flow=public-flow","verify_url":"https://example.invalid/account?flow=public-flow","expires_in":600,"interval":3}"#.to_string(),
        )]);
        let result = request_device_authorization_impl(&client(), &base, Some(JcodeTier::Pro))
            .await
            .expect("device auth");
    ...
    async fn legacy_device_response_is_explained_without_echoing_body() {
        ...
        let error = request_device_authorization_impl(&client(), &base, Some(JcodeTier::Pro))
            .await
    ...
    async fn token_poll_handles_pending_slow_down_success_denied_and_replay() {
        ...
        let client = client();
        assert_eq!(
            poll_device_token_once_impl(&client, &base, "secret")
                .await
                .unwrap(),
    ...
    async fn me_and_revoke_classify_revoked_keys_without_leaking_them() {
        ...
        let client = client();
        let me_error = fetch_subscription_me_with_impl(&client, &base, "jck_live_secret")
            .await
            .expect_err("revoked");
        assert_eq!(me_error, AccountApiError::Unauthorized);
        assert!(!me_error.to_string().contains("jck_live"));
        let revoke_error = revoke_current_key_impl(&client, &base, "jck_live_secret")
            .await

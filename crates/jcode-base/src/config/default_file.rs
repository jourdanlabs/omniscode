...
# session's working directory. OMNIS KEY V1 ships no global-hotkey installer;
# inherited setup-hotkey behavior is disabled.
...
# Inherited server updater/reload behavior is disabled in OMNIS KEY Local Integrity V1.
...
# Retired compatibility field. OMNIS KEY V1 performs no update discovery,
# download, or self-replacement; this value is parsed but has no update effect.
...
# focus_hook = ""
#
# macOS only: terminal used for explicit in-app session spawns. One of:
# ghostty, iterm2, wezterm, warp, alacritty, vscode, terminal (Apple Terminal).
# The inherited global-hotkey installer is disabled in OMNIS KEY V1.
# preferred = "ghostty"
...
# Optional third-party tool discovery. OMNIS KEY ships no discovery endpoint.
# Both fields must be supplied deliberately to register `discover_tools`.
# enabled = false
# endpoint = "https://discovery.example/v1"
	"#;

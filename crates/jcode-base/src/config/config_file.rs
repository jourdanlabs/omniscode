...
fn inherited_launch_hotkey_bake_disabled() -> bool {
    true
}
...
impl Config {
    ...
    /// Apply the V1 discovery boundary after file and environment loading.
    pub(crate) fn enforce_sponsors_sovereignty(&mut self) {
        if self.sponsors.enabled
            && !super::sponsors_endpoint_is_allowed(self.sponsors.endpoint.trim())
        {
    ...
    pub fn save(&self) -> anyhow::Result<()> {
        if self.sponsors.enabled
            && !super::sponsors_endpoint_is_allowed(self.sponsors.endpoint.trim())
        {
            anyhow::bail!("tool discovery requires an explicit non-Jcode HTTP(S) endpoint");
        }
    ...
    pub fn bake_launch_hotkeys_once() -> bool {
        if inherited_launch_hotkey_bake_disabled() {
            return false;
        }

...
impl AuthStatus {
    ...
    /// Cached-only auth snapshot for per-frame render paths (the TUI header).
    ///
    /// Rendering must not inspect credential files or start a probe thread.
    /// Explicit auth/login actions populate or invalidate the normal caches;
    /// until then the first frame reports the conservative default state.
    pub fn check_fast_nonblocking() -> Self {
        ...
        let Some(status) = AUTH_STATUS_FAST_CACHE
            .read()
            .ok()
    ...
    pub fn state_for_provider(&self, provider: LoginProviderDescriptor) -> AuthState {
        match provider.target {
            ...
            crate::provider_catalog::LoginProviderTarget::Jcode => self.jcode,
            crate::provider_catalog::LoginProviderTarget::OpenRouter => self.openrouter,
            crate::provider_catalog::LoginProviderTarget::OpenAiApiKey => {
                if self.openai_has_api_key {
                    AuthState::Available
                } else {
                    AuthState::NotConfigured
                }
            ...
            crate::provider_catalog::LoginProviderTarget::Bedrock => self.bedrock,

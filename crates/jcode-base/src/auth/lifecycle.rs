...
impl AuthActivationRequest {
    ...
    pub fn requests_disabled_jcode_subscription(&self) -> bool {
        super::sovereignty::requests_disabled_jcode_subscription(
            self.legacy_provider_hint.as_deref(),
            self.auth.as_ref(),
        )
    }
}
...
pub fn activate_auth_change(request: &AuthActivationRequest) -> AuthActivationResult {
    if request.requests_disabled_jcode_subscription() {
        return AuthActivationResult::default();
    }
...
mod tests {
    ...
    fn direct_login_provider_descriptor_matrix_has_full_lifecycle_parity() {
        ...
        for provider in crate::provider_catalog::login_providers() {
            let Some((normalized, runtime, active, switch_prefix)) = (match provider.target {
                crate::provider_catalog::LoginProviderTarget::Jcode => None,
    ...
    #[test]
    fn jcode_auth_lifecycle_aliases_and_typed_metadata_are_inert() {
        let _guard = crate::storage::lock_test_env();
        crate::env::set_var("JCODE_RUNTIME_PROVIDER", "sentinel-runtime");
        crate::env::set_var("JCODE_ACTIVE_PROVIDER", "sentinel-provider");

        for request in [
            AuthActivationRequest::new(Some("jcode".to_string()), None),
            AuthActivationRequest::new(Some("subscription".to_string()), None),
            AuthActivationRequest::new(Some("jcode-subscription".to_string()), None),
            AuthActivationRequest::new(None, Some(AuthChanged::new("jcode"))),
        ] {
            assert!(request.requests_disabled_jcode_subscription());
            assert_eq!(request.provider_id(), None);
            assert_eq!(
                activate_auth_change(&request),
                AuthActivationResult::default()
            );
        }

        let mut metadata_only = AuthChanged::new("openai");
        metadata_only.expected_runtime = Some(RuntimeProviderKey::new("jcode-subscription"));
        metadata_only.expected_catalog_namespace =
            Some(CatalogNamespace::new("jcode-subscription"));
        let metadata_request = AuthActivationRequest::new(None, Some(metadata_only));
        assert!(metadata_request.requests_disabled_jcode_subscription());
        assert_eq!(
            activate_auth_change(&metadata_request),
            AuthActivationResult::default()
        );
        assert_eq!(
            std::env::var("JCODE_RUNTIME_PROVIDER").as_deref(),

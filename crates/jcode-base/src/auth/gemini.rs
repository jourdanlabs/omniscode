...
// Gemini OAuth is bring-your-own-client. Upstream embedded a third party's
// "Desktop app" credentials here on the theory that an installed-app client
// secret is not confidential (RFC 8252). That is true of the protocol and still
// the wrong thing for this repository to distribute: the credentials belong to
// someone else, and a project that ships receipts should not ask anyone to take
// "it's fine, it's public" on faith. Configure your own client instead:
...
// No embedded fallback: an unset variable is an error the caller must surface,
// not a default that silently authenticates as somebody else's application.
fn required_oauth_env(var: &str) -> Result<String> {
    ...
    value.ok_or_else(|| {
        anyhow::anyhow!(
            "{var} is not set. Gemini OAuth requires your own Google \"Desktop app\" \
             OAuth client: set {GEMINI_CLIENT_ID_ENV} and {GEMINI_CLIENT_SECRET_ENV}. \
...
fn gemini_client_id() -> Result<String> {
    required_oauth_env(GEMINI_CLIENT_ID_ENV)
}

fn gemini_client_secret() -> Result<String> {
    required_oauth_env(GEMINI_CLIENT_SECRET_ENV)
}

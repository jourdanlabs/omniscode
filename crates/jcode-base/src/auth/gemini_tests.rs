...
// Standing proof that no vendor OAuth client is embedded in this crate.
// Reintroducing a hardcoded fallback makes this test fail: the URL would build
// with no credential configured.
#[test]
fn errors_when_credential_env_missing() {
    let _guard = lock_test_env();
    crate::env::remove_var(GEMINI_CLIENT_ID_ENV);
    crate::env::remove_var(GEMINI_CLIENT_SECRET_ENV);
    ...
    assert!(
        err.to_string().contains(GEMINI_CLIENT_ID_ENV),
        "error must name the variable to set, got: {err}"
    );
}

#[test]
fn builds_auth_url_from_configured_credentials() {
    let _guard = lock_test_env();
    crate::env::set_var(GEMINI_CLIENT_ID_ENV, "test-client-id.example");
    crate::env::set_var(GEMINI_CLIENT_SECRET_ENV, "test-client-secret");

    let url = build_manual_auth_url(GEMINI_MANUAL_REDIRECT_URI, "challenge-123", "state-123")
        .expect("configured credentials should build a URL");
    assert!(url.contains("codeassist.google.com%2Fauthcode"));
    assert!(url.contains("code_challenge=challenge-123"));
    assert!(url.contains("client_id=test-client-id.example"));

...
pub fn auth_provider_integrations() -> Vec<AuthProviderIntegration> {
    login_providers()
        .iter()
        .copied()
        .filter(|descriptor| descriptor.target != LoginProviderTarget::Jcode)
...
mod tests {
    ...
    fn integration_registry_covers_login_catalog_once() {
        ...
        let descriptors = login_providers()
            .iter()
            .copied()
            .filter(|descriptor| descriptor.target != LoginProviderTarget::Jcode)
            .collect::<Vec<_>>();
        assert_eq!(integrations.len(), descriptors.len());

        let mut ids = HashSet::new();
        for (integration, descriptor) in integrations.iter().zip(&descriptors) {
    ...
    #[test]
    fn inherited_jcode_subscription_is_absent_from_integration_registry() {
        assert!(auth_provider_integration("jcode").is_none());
        assert!(auth_provider_integration("subscription").is_none());
        assert!(
            auth_provider_integrations()
                .iter()
                .all(|integration| integration.descriptor.target != LoginProviderTarget::Jcode)
        );
    }

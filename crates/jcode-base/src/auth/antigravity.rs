...
// Antigravity OAuth is bring-your-own-client. Upstream embedded credentials
// belonging to another vendor's desktop app; this build does not redistribute
// them. Set both variables to your own OAuth client to use this provider.
...
// No embedded fallback: a blank or unset variable is an error the caller must
// surface, not a default that silently authenticates as somebody else's app.
fn required_oauth_env(var: &str) -> Result<String> {
    std::env::var(var)
        .ok()
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "{var} is not set. Antigravity OAuth requires your own Google \
                 \"Desktop app\" OAuth client: set {CLIENT_ID_ENV} and {CLIENT_SECRET_ENV}."
            )
        })
}
...
mod tests {
    ...
    // These two tests are the standing proof that no vendor OAuth client is
    // embedded in this crate. If someone reintroduces a hardcoded fallback,
    // both fail — the auth URL would build, and the getters would succeed,
    // with no credential configured.
    #[test]
    fn build_auth_url_errors_when_client_id_env_missing() {
        let _guard = lock_test_env();
        crate::env::remove_var(CLIENT_ID_ENV);
        ...
        .expect_err("no embedded client id — missing env must be an error");
        assert!(
            err.to_string().contains(CLIENT_ID_ENV),
            "error must name the variable to set, got: {err}"
        );
    }

    #[test]
    fn blank_env_vars_are_rejected_not_defaulted() {
        let _guard = lock_test_env();

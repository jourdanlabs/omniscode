...
/// Get a read-only usage snapshot synchronously.
///
/// Ambient refresh is forbidden: missing or stale state stays local until an
/// explicit usage/provider action calls the async refresh entry.

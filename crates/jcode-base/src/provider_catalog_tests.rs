...
fn matrix_login_provider_aliases_resolve_to_canonical_ids() {
    assert!(resolve_login_provider("jcode").is_none());
    assert!(resolve_login_provider("subscription").is_none());
    assert_eq!(
...
fn matrix_tui_login_selection_supports_numbers_and_names() {
    ...
    // `anthropic-api` remains third while the inherited Jcode subscription
    // entry is quarantined from every login surface.
    assert_eq!(
...
fn matrix_cli_login_selection_preserves_existing_order() {
    ...
    // The inherited Jcode subscription entry is quarantined, so the public
    // numeric grammar closes that slot without exposing an inert provider.
    assert_eq!(
        resolve_login_selection("3", &providers).map(|provider| provider.id),
        Some("anthropic-api")
    );

...
#[test]
fn deferred_startup_ignores_config_file_and_environment_overrides() {
    let _guard = crate::storage::lock_test_env();
    let prev_home = std::env::var_os("JCODE_HOME");
    let prev_deferred = std::env::var_os("JCODE_DEFERRED_AUTH_BOOTSTRAP");
    let prev_centered = std::env::var_os("JCODE_DISPLAY_CENTERED");
    let dir = tempfile::TempDir::new().expect("tempdir");
    ...
    .expect("write ambient config fixture");

    assert!(!Config::load().display.centered);
    assert!(
        !Config::load_strict()
            .expect("deferred strict load")
            .display
            .centered
    );

    restore_env_var("JCODE_HOME", prev_home);
    restore_env_var("JCODE_DEFERRED_AUTH_BOOTSTRAP", prev_deferred);
    restore_env_var("JCODE_DISPLAY_CENTERED", prev_centered);
}
...
#[test]
fn inherited_discovery_endpoint_is_disabled_on_load() {
    let _guard = crate::storage::lock_test_env();
    let prev_home = std::env::var_os("JCODE_HOME");
    let dir = tempfile::TempDir::new().expect("tempdir");
    crate::env::set_var("JCODE_HOME", dir.path());
    ...
    assert!(
        ...
        "the inherited discovery service must never be activated"
    );
    assert!(
        loaded.sponsors.endpoint.is_empty(),
        "the inherited endpoint must be removed from the runtime config"
    );
...
#[test]
fn explicit_custom_discovery_endpoint_can_be_enabled() {
    let raw = "[sponsors]\nenabled = true\nendpoint = \"https://discovery.internal/v1\"\n";
    let mut config: Config = toml::from_str(raw).expect("parse");
    config.enforce_sponsors_sovereignty();
    assert!(config.sponsors.enabled);
    assert!(super::sponsors_endpoint_is_allowed(
        &config.sponsors.endpoint
    ));
}

#[test]
fn enabled_discovery_without_an_explicit_endpoint_is_disabled() {
    let raw = "[sponsors]\nenabled = true\n";
    let mut config: Config = toml::from_str(raw).expect("parse");
    config.enforce_sponsors_sovereignty();
    assert!(!config.sponsors.enabled);
...
#[path = "config_sovereignty_tests.rs"]

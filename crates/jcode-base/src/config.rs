...
fn config_env_fingerprint() -> Vec<(String, String)> {
    let mut values = CONFIG_ENV_KEYS
        .iter()
        .filter_map(|key| {
            std::env::var_os(key)
                .map(|value| ((*key).to_owned(), value.to_string_lossy().to_string()))
        })
        .collect::<Vec<_>>();
...
fn sponsors_is_default(sponsors: &SponsorsConfig) -> bool {
    !sponsors.enabled && sponsors.endpoint.trim().is_empty()
}

/// Whether an explicitly configured discovery endpoint is allowed in the
/// sovereign fork. Inherited Jcode/Solo Systems services are never valid
/// targets, even if they remain in a legacy config file.
pub fn sponsors_endpoint_is_allowed(endpoint: &str) -> bool {
    ...
    !matches!(
        ...
        "jcode.sh"
            ...
            | "www.solosystems.dev"
            | "api.solosystems.dev"
    ) && !host.ends_with(".jcode.sh")
        && !host.ends_with(".solosystems.dev")
}

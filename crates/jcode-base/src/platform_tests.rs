...
fn spawn_detached_creates_new_session() {
    ...
    child.kill().expect("terminate detached test child");
    child.wait().expect("reap detached test child");
    ...
    assert_ne!(child_sid, parent_sid, "child must not share parent session");
}

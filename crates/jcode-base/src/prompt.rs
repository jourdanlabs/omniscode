...
fn build_selfdev_prompt_static() -> String {
    build_selfdev_prompt_static_for_working_dir(None)
}

/// Build self-dev tools prompt section
#[cfg(test)]
fn build_selfdev_prompt() -> String {
    build_selfdev_prompt_for_working_dir(None)
}

fn build_selfdev_prompt_static_for_working_dir(_working_dir: Option<&Path>) -> String {
    build_selfdev_prompt_for_working_dir(None).replace("__DEBUG_SOCKET_BLOCK__\n\n", "")
}

fn build_selfdev_prompt_for_working_dir(_working_dir: Option<&Path>) -> String {
    SELFDEV_MODE_PROMPT.replace("__SELFDEV_PRODUCT_FOCUS__", SELFDEV_FOCUS_TUI_PROMPT)
}
...
fn get_git_info(_working_dir: Option<&Path>) -> Option<String> {
    // Prompt assembly is part of minimal startup and must never execute a
    // PATH-resolved helper. Git inspection remains an explicit tool action.
    None
}
...
fn gpu_summary() -> Option<String> {
    // Avoid PATH-resolved hardware probes during prompt construction.
    None
}

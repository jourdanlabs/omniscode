...
pub(crate) fn openai_effective_auth_mode() -> &'static str {
    // The credential-free startup shell must not inspect OAuth files or API-key
    // environment variables merely to price placeholder/model-picker routes.
    // Explicit auth and provider actions run outside this deferred bootstrap
    // path and repopulate the pricing memo after credentials actually change.
...
///   3. A locally available models.dev-format pricing cache.
///
...
pub fn metered_pricing_for_source_with_tier(
    ...
    // 3. Local models.dev-format disk cache. Lookups are read-only and never
    // schedule a refresh.
...
mod tests {
    ...
    fn with_clean_provider_test_env<T>(f: impl FnOnce() -> T) -> T {
        ...
        let prev_deferred_auth = std::env::var_os("JCODE_DEFERRED_AUTH_BOOTSTRAP");
        crate::auth::claude::set_active_account_override(None);
        crate::auth::codex::set_active_account_override(None);
        env::set_var("JCODE_HOME", temp.path());
        env::remove_var("OPENAI_API_KEY");
        env::remove_var("JCODE_COPILOT_PREMIUM");
        env::remove_var("JCODE_DEFERRED_AUTH_BOOTSTRAP");
        ...
        if let Some(prev_deferred_auth) = prev_deferred_auth {
            env::set_var("JCODE_DEFERRED_AUTH_BOOTSTRAP", prev_deferred_auth);
        } else {
            env::remove_var("JCODE_DEFERRED_AUTH_BOOTSTRAP");
        }
        result
    ...
    #[test]
    fn deferred_startup_pricing_does_not_activate_ambient_openai_api_key() {
        with_clean_provider_test_env(|| {
            env::set_var("OPENAI_API_KEY", "fixture-only-ambient-key");
            env::set_var("JCODE_DEFERRED_AUTH_BOOTSTRAP", "1");
            let estimate = cheapness_for_route("gpt-5.4", "OpenAI", "openai-oauth")
                .expect("cheapness estimate");
            assert_eq!(estimate.billing_kind, RouteBillingKind::Subscription);
            assert_eq!(estimate.source, RouteCostSource::PublicPlanPricing);

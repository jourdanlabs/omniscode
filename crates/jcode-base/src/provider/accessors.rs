...
impl MultiProvider {
    ...
    pub(super) fn reconcile_auth_if_provider_missing(&self, provider: ActiveProvider) -> bool {
        // Missing-provider checks are pure. Credential/account discovery is
        // allowed only through an explicit login/auth-change operation; a
        // model lookup, session fork, render, or failed completion must never
        // turn into an ambient credential scan.

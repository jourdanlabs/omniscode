...
fn empty_multi_provider_template() -> MultiProvider {
    MultiProvider {
...
#[test]
fn openai_model_routes_follow_runtime_snapshot_not_ambient_auth_state() {
    with_clean_provider_test_env(|| {
        let rt = enter_test_runtime();
        let _runtime_guard = rt.enter();
        let model = known_openai_model_ids()
        ...
        let methods = routes
            ...
            .collect::<Vec<_>>();
        assert_eq!(methods, vec![("current", true)]);
        ...
        assert_eq!(
            oauth_only_methods,
            vec!["current"],
            "removing ambient API credentials must not rewrite an initialized runtime snapshot"
        );
        ...
        assert_eq!(
            api_only_methods,
            vec!["current"],
            "removing an external OAuth account must not rewrite an initialized runtime snapshot"
        );

        provider
            .set_model(&format!("openai-oauth:{model}"))
            .expect("pin initialized runtime to OAuth");
        ...
        provider
            .set_model(&format!("openai-api:{model}"))
            .expect("pin initialized runtime to API key");
        assert!(provider.model_routes().iter().any(|route| {
            route.provider == "OpenAI"
                && route.model == model
...
#[test]
fn empty_provider_template_route_snapshot_ignores_ambient_credentials_and_factories() {
    with_clean_provider_test_env(|| {
        struct RestoreExternalFactories;
        impl Drop for RestoreExternalFactories {
            fn drop(&mut self) {
                register_test_external_runtimes();
            }
        }
        let _restore_factories = RestoreExternalFactories;

        let factory_calls = Arc::new(std::sync::atomic::AtomicUsize::new(0));
        for runtime in [
            external::ANTHROPIC_RUNTIME,
            external::OPENAI_RUNTIME,
            external::COPILOT_RUNTIME,
            external::ANTIGRAVITY_RUNTIME,
            external::CURSOR_RUNTIME,
        ] {
            let factory_calls = Arc::clone(&factory_calls);
            external::register_external_provider(runtime, move || {
                factory_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
                Arc::new(StubExternalRuntime::cursor()) as Arc<dyn Provider>
            });
        }
        let openrouter_factory_calls = Arc::clone(&factory_calls);
        external::register_openrouter_factory(move |_| {
            openrouter_factory_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            anyhow::bail!("route enumeration must not instantiate OpenRouter")
        });

        // Seed every ambient source that the inherited route builder used to
        // probe: direct keys, a saved external account, and a configured
        // OpenAI-compatible profile. None may create a route without a runtime
        // already installed in the template.
        crate::env::set_var("ANTHROPIC_API_KEY", "sk-ant-ambient-canary");
        crate::env::set_var("OPENAI_API_KEY", "sk-openai-ambient-canary");
        crate::env::set_var("OPENROUTER_API_KEY", "sk-or-ambient-canary");
        crate::env::set_var("OPENCODE_API_KEY", "oc-ambient-canary");
        crate::env::set_var(
            "JCODE_OPENAI_COMPAT_API_BASE",
            "https://ambient-profile.invalid/v1",
        );
        crate::env::set_var("OPENAI_COMPAT_API_KEY", "sk-compat-ambient-canary");
        crate::env::set_var("JCODE_OPENAI_COMPAT_DEFAULT_MODEL", "ambient-profile-model");
        save_test_openai_oauth_credentials();
        crate::auth::AuthStatus::invalidate_cache();

        let provider = empty_multi_provider_template();
        let routes = super::catalog_routes::multiprovider_model_routes(&provider);

        assert!(
            routes.is_empty(),
            "an empty provider template must remain empty despite ambient credentials/accounts/config: {routes:?}"
        );
        assert_eq!(
            factory_calls.load(std::sync::atomic::Ordering::SeqCst),
            0,
            "route enumeration must not instantiate any external provider"
        );
        assert!(!provider.has_claude_runtime());
        assert!(provider.openai_provider().is_none());
        assert!(provider.copilot_provider().is_none());
        assert!(provider.openrouter_provider().is_none());
        assert!(
            provider
                .openai_compatible_profiles
                .read()
                .expect("compatible profile slots")
                .is_empty()
        );
    });
}
...
#[test]
fn route_snapshot_contains_only_an_initialized_provider_slot() {
    with_clean_provider_test_env(|| {
        ...
        assert!(
            !routes.is_empty(),
            "initialized cursor slot should contribute routes"
        );
        assert!(
            routes
                .iter()
                .all(|route| route.provider == "Cursor" && route.api_method == "cursor"),
            "no uninitialized provider family may be synthesized: {routes:?}"
        );
    });
}
...
#[test]
fn explicitly_initialized_openai_compatible_profile_routes_use_live_cache() {
    with_clean_provider_test_env(|| {
        crate::provider_catalog::save_env_value_to_env_file(
            "OPENROUTER_API_KEY",
            "openrouter.env",
        ...
        assert!(
            provider
                .model_routes()
                .iter()
                .all(|route| route.provider != "OpenCode Zen"),
            "saved credentials alone must not initialize an ambient profile"
        );
        provider
            .set_model("opencode:zen-live-only-model")
            .expect("explicit OpenCode selection should install its runtime");
        let routes = provider.model_routes();
        ...
        assert!(
            ...
            "explicitly initialized direct profile should expose its live /models cache, routes: {opencode_routes:?}"
        );
        assert!(
            opencode_routes
                .iter()
                .any(|route| route.model == "kimi-k2.6"),
            "explicitly initialized runtime must read its profile-scoped cache: {opencode_routes:?}"
        );
        assert!(
...
#[test]
fn new_session_fork_preserves_template_despite_changed_config() {
    with_clean_provider_test_env(|| {
        ...
        runtime.block_on(async {
            ...
            assert_eq!(fresh.model(), "claude-fable-5");
...
include!("tests/model_resolution_v1_guardrails.rs");

...
#[test]
fn test_subscription_model_guard_stays_inert_after_hostile_activation_attempt() {
    let _guard = crate::storage::lock_test_env();
...
#[test]
fn test_subscription_tier_cannot_activate_model_gating() {
    let _guard = crate::storage::lock_test_env();
    crate::subscription_catalog::clear_runtime_env();
    crate::env::set_var(
        crate::subscription_catalog::JCODE_SUBSCRIPTION_ACTIVE_ENV,
...
#[test]
fn test_filtered_display_models_ignore_disabled_subscription_catalog() {
    let _guard = crate::storage::lock_test_env();
    crate::subscription_catalog::clear_runtime_env();
    crate::env::set_var(
        crate::subscription_catalog::JCODE_SUBSCRIPTION_ACTIVE_ENV,
    ...
    assert_eq!(filtered_display_models(models.clone()), models);
...
#[test]
fn test_hostile_remote_jcode_subscription_catalog_is_quarantined() {
    let models = vec![
        "claude-opus-4-8".to_string(),
        "claude-sonnet-4-6".to_string(),
        "gpt-5.5".to_string(),
        "gpt-5.6-sol".to_string(),
        "attacker/fabricated-premium-model".to_string(),
    ];
    ...
    assert!(
        routes.is_empty(),
        "a hostile remote catalog must not synthesize inherited subscription routes: {routes:?}"
    );

    let lightweight = remote_model_routes_lightweight_fallback(
        Some(crate::subscription_catalog::JCODE_PROVIDER_DISPLAY_NAME),
        &models,
        "attacker/current-premium-model",
    );
    assert!(
        lightweight.is_empty(),
        "lightweight/current-model fallback must also quarantine inherited subscription identity: {lightweight:?}"
    );
}

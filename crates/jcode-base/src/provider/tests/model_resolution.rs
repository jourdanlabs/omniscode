...
fn test_available_models_display_uses_route_models_and_filters_placeholder_rows() {
    ...
    with_clean_provider_test_env(|| {
        let provider = MultiProvider {
            ...
            openai: RwLock::new(Some(test_openai_runtime())),
    ...
    for (model, provider_key, api_method, expected) in cases {
        assert_eq!(
            MultiProvider::model_switch_request_for_session_route(model, provider_key, api_method)
                .expect("valid session route should restore"),
            expected,
...
#[test]
fn test_explicitly_initialized_direct_compatible_profiles_are_listed_without_openrouter_key() {
    with_clean_provider_test_env(|| {
        with_env_var("DEEPSEEK_API_KEY", "test-deepseek-key", || {
            with_env_var("KIMI_API_KEY", "test-kimi-key", || {
                ...
                assert!(
                    ...
                    "credentials alone must not initialize ambient provider routes"
                );
                provider
                    .set_model("deepseek:deepseek-v4-flash")
                    .expect("explicit DeepSeek selection should install its runtime");
                provider
                    .set_model("kimi:kimi-for-coding")
                    .expect("explicit Kimi selection should install its runtime");
                let routes = provider.model_routes();
                assert!(routes.iter().any(|route| {
...
"#,
        ...
        assert!(
            provider.model_routes().is_empty(),
            "config alone must not initialize an ambient provider route"
        );

        // Explicit selection binds the named profile runtime. The picker can
        // then render its text-capable model while excluding image-only rows.
        provider
...
"#
    ...
    provider
        .set_model("my-gateway:opaque/model@id")
        .expect("explicit named-profile selection");
    assert_eq!(provider.active_provider(), ActiveProvider::OpenRouter);
    let openrouter = provider
        .active_openrouter_execution_provider()
        .expect("active named-profile execution runtime");
    assert!(
...
fn bare_openai_compatible_model_id_routes_to_its_profile_not_the_active_provider() {
    with_clean_provider_test_env(|| {
        ...
        provider
            .set_model("celeris:celeris-1")
            .expect("explicit Celeris selection should install its runtime");
        provider.clear_active_openai_compatible_profile();

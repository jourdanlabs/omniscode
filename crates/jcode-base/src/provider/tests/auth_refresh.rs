...
fn test_on_auth_changed_hot_initializes_openai_and_marks_routes_available() {
    with_clean_provider_test_env(|| {
        ...
        assert!(provider.model_routes().iter().any(|route| {
            route.provider == "OpenAI" && route.api_method == "current" && route.available
        }));
    });
}
...
fn test_on_auth_changed_hot_initializes_anthropic_and_marks_routes_available() {
    with_clean_provider_test_env(|| {
        ...
        assert!(provider.model_routes().iter().any(|route| {
            route.provider == "Anthropic" && route.api_method == "current" && route.available
        }));
    });
}
...
#[test]
fn test_anthropic_model_routes_do_not_read_external_account_entitlements() {
    with_clean_provider_test_env(|| {
        let runtime = enter_test_runtime();
        let _enter = runtime.enter();
        ...
        assert!(
            opus_1m.detail.is_empty(),
            "route enumeration must not derive entitlement details from ambient account state"
        );
    });
}
...
#[test]
fn test_startup_ignores_expired_antigravity_tokens() {
    with_clean_provider_test_env(|| {
        let runtime = enter_test_runtime();
        let _enter = runtime.enter();
        ...
        assert!(provider.antigravity_provider().is_none());
        assert!(
            provider
                .model_routes()
                .iter()
                .all(|route| route.provider != "Antigravity")
        );
    });
}

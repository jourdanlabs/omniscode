...
impl MultiProvider {
    fn new_deferred() -> Self {
        Self {
            ...
            active: RwLock::new(Self::auto_default_provider(ProviderAvailability::default())),
    ...
    pub(super) fn new_with_auth_status(auth_status: auth::AuthStatus) -> Self {
        // No-argument startup is a credential-less local shell. Do not let an
        // ambient default provider (including a no-auth localhost profile)
        // turn construction into provider selection, config activation, or
        // catalog discovery before the operator explicitly chooses a route.
        if std::env::var_os("JCODE_DEFERRED_AUTH_BOOTSTRAP").is_some() {
            return Self::new_deferred();
        }
        ...
        // Construction consumes the caller-supplied snapshot only. Re-probing
        // credential stores here made an otherwise empty/no-argument startup
        // inspect account files a second time.
        let has_claude_creds = auth_status.anthropic.has_oauth || auth_status.anthropic.has_api_key;
        let has_openai_creds = auth_status.openai_has_oauth || auth_status.openai_has_api_key;
        let has_copilot_api = provider_state.auth_status().copilot_has_api_token;
        let has_antigravity_creds = auth_status.antigravity == auth::AuthState::Available;
        let has_gemini_creds = auth_status.gemini == auth::AuthState::Available;
        let has_cursor_creds = auth_status.cursor == auth::AuthState::Available;
        let has_bedrock_creds = auth_status.bedrock == auth::AuthState::Available;
        let has_openrouter_creds = auth_status.openrouter == auth::AuthState::Available;
    ...
    pub(super) fn spawn_openai_catalog_refresh_if_needed(&self) {
        // Inherited constructor/render-time catalog discovery is disabled.
        // Explicit auth/provider refresh paths use `spawn_post_auth_model_refresh`.
    }
    ...
    pub fn new_fast() -> Self {
        Self::new_with_auth_status(auth::AuthStatus::check_fast_nonblocking())
    }

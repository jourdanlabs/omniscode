...
impl ProviderActivation {
    ...
    pub fn apply_env(&self) -> Result<()> {
        ...
        match self.runtime_id {
            ...
            RuntimeProviderId::Jcode => unreachable!("disabled Jcode activation returned above"),
            _ => {
...
mod tests {
    ...
    #[test]
    fn inherited_jcode_activation_refuses_before_any_env_mutation() {
        let _lock = crate::storage::lock_test_env();
        let _guard = EnvGuard::new(&[
            "JCODE_RUNTIME_PROVIDER",
            "JCODE_ACTIVE_PROVIDER",
        ...
        let constructor_error = ProviderActivation::jcode_subscription("hostile-model")
            .expect_err("Jcode subscription activation must refuse");
        assert_eq!(
        ...
        let direct =
            ProviderActivation::initial(RuntimeProviderId::Jcode, ActiveProvider::OpenRouter)
                .with_model_hint("JCODE_OPENROUTER_MODEL", "hostile-model");
        let direct_error = direct
            .apply_env()
            .expect_err("directly constructed Jcode activation must refuse");
        assert_eq!(
        ...
        assert!(std::env::var_os("JCODE_INITIAL_PROVIDER_EXPLICIT").is_none());
        assert!(std::env::var_os("JCODE_OPENROUTER_TRANSPORT_STATE").is_none());
    }
}

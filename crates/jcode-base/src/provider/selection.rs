...
impl MultiProvider {
    ...
    pub fn model_switch_request_for_session_route(
        ...
        let model_prefix = model
            ...
            .unwrap_or("");
        if provider_key
            .is_some_and(jcode_provider_core::sovereignty::is_inherited_jcode_subscription_alias)
            || route_api_method.is_some_and(
                jcode_provider_core::sovereignty::is_inherited_jcode_subscription_alias,
            )
            || jcode_provider_core::sovereignty::is_inherited_jcode_subscription_alias(model_prefix)
        {
            ...
            .filter(|api_method| !api_method.is_empty())
        {
            match ModelRouteApiMethod::parse(api_method) {
                ModelRouteApiMethod::JcodeSubscription => anyhow::bail!(
                    crate::subscription_catalog::INHERITED_SUBSCRIPTION_DISABLED_MESSAGE
                ),
                ModelRouteApiMethod::ClaudeOAuth => {
                    return Ok(format!("claude-oauth:{model}"));
                }
                ModelRouteApiMethod::AnthropicApiKey => {
                    return Ok(format!("claude-api:{model}"));
                }
                ModelRouteApiMethod::OpenAIOAuth => {
                    return Ok(format!("openai-oauth:{model}"));
                }
                ModelRouteApiMethod::OpenAIApiKey => {
                    return Ok(format!("openai-api:{model}"));
                }
                ModelRouteApiMethod::OpenRouter => return Ok(format!("openrouter:{model}")),
                ModelRouteApiMethod::OpenAiCompatible {
                    profile_id: Some(profile_id),
                } => return Ok(format!("{profile_id}:{model}")),
                ModelRouteApiMethod::Copilot => return Ok(format!("copilot:{model}")),
                ModelRouteApiMethod::Cursor => return Ok(format!("cursor:{model}")),
                ModelRouteApiMethod::Bedrock => return Ok(format!("bedrock:{model}")),
                ModelRouteApiMethod::AntigravityHttps => {
                    return Ok(format!("antigravity:{model}"));
                }
                ModelRouteApiMethod::OpenAiCompatible { profile_id: None }
        ...
        Ok(Self::model_switch_request_for_session_model(
            model,
            provider_key,
        ))
    }
...
mod tests {
    ...
    fn session_model_route_identity_helpers_preserve_auth_mode_and_profiles() {
        ...
        assert_eq!(
            ...
            .expect("OpenRouter session route should restore"),
        ...
        assert_eq!(
            ...
            .expect("OpenAI-compatible session route should restore"),
    ...
    fn bare_session_provider_key_restores_auto_credential_not_oauth_pin() {
        ...
        assert_eq!(
            ...
            .expect("bare Claude session route should restore"),
        ...
        assert_eq!(
            ...
            .expect("Claude API session route should restore"),
        ...
        assert_eq!(
            ...
            .expect("Claude OAuth session route should restore"),
            "claude-oauth:claude-opus-5"
        );
    }

    #[test]
    fn persisted_jcode_subscription_route_identity_is_refused() {
        for (model, provider_key, api_method) in [
            ("fixture-model", Some("JCODE"), None),
            ("fixture-model", None, Some("jcode_subscription")),
            ("fixture-model", Some("Jcode Subscription"), Some("current")),
            ("jcode:fixture-model", None, None),
        ] {
            let error = MultiProvider::model_switch_request_for_session_route(
                model,
                provider_key,
                api_method,
            )
            .expect_err("inherited persisted route must refuse");
            assert_eq!(
                error.to_string(),
                crate::subscription_catalog::INHERITED_SUBSCRIPTION_DISABLED_MESSAGE
            );

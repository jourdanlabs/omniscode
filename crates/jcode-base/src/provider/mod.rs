...
impl Provider for MultiProvider {
    ...
    fn fork(&self) -> Arc<dyn Provider> {
        ...
        if provider.provider_slot_available(active) {
    ...
    fn fork_for_new_session(&self) -> Arc<dyn Provider> {
        // Preserve the server template exactly. Reconstructing from ambient
        // auth/config state here made a no-argument client attachment scan
        // credentials and activate providers behind the user's back.
        self.fork()
    }

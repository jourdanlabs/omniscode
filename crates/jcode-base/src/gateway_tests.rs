...
fn fixture_hex_token(seed: u8) -> String {
    // Built at runtime so the tree never embeds a scanner-shaped secret constant.
    (0..64)
        .map(|i| format!("{:x}", (usize::from(seed) + i) % 16))
        .collect()
}

#[test]
fn test_hex_token_validation() {
    assert!(is_valid_hex_token(&"0".repeat(64)));
    assert!(!is_valid_hex_token("abc"));
    assert!(!is_valid_hex_token(&"z".repeat(64)));
}

#[test]
fn test_extract_ws_auth_prefers_header_and_falls_back_to_query() {
    let token_a = fixture_hex_token(0);
    let token_b = fixture_hex_token(15);

...
//!
//! Hybrid retrieval (`MemoryManager::find_similar_hybrid`) produces a top-N
//! candidate pool. A reranker can reorder that pool before surfacing results.
//!
...
//! system-reminder/tool noise stripped) and all candidates in one call.
//!
//! This module is the single source of truth for that reranking, shared by the
//! offline test harness and the memory agent. It is pure with respect to the

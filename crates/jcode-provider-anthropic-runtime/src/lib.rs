...
use jcode_provider_core::anthropic_is_1m_model as is_1m_model;
use jcode_provider_core::anthropic_map_tool_name_from_oauth as map_tool_name_from_oauth;
use jcode_provider_core::anthropic_strip_1m_suffix as strip_1m_suffix;
use reqwest::Client;
use serde::Serialize;
...
fn anthropic_recommended_model_from_error(error_str: &str) -> Option<String> {
    ...
    // separators such as the dot in "Opus 4.8".
    let sentence_end = hint.char_indices().find_map(|(index, character)| {
        ...
        let previous_is_digit = hint.as_bytes()[..index]
            .last()
            .is_some_and(u8::is_ascii_digit);
        let next_is_digit = hint.as_bytes()[index + character.len_utf8()..]
            .first()
            .is_some_and(u8::is_ascii_digit);
        (!previous_is_digit || !next_is_digit).then_some(index)
    });

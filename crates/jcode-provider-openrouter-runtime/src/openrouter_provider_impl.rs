...
impl Provider for OpenRouterProvider {
    ...
    async fn prefetch_models(&self) -> Result<()> {
        // Startup and post-auth initialization remain local-only. Users can
        // request a catalog refresh explicitly when they want remote discovery.
        Ok(())
    }
...
impl OpenRouterProvider {
    ...
    /// writes should use.
    ///
    /// A profile runtime carries its own stable identity, so foreground reads
    /// must not depend on the last process-global namespace written by some
    /// other profile. This covers both user-named and built-in profiles selected
    /// explicitly. Real OpenRouter has no profile id and keeps the standard
    /// `openrouter_models.json` path. See issue #607.
    ...
    /// namespace env var for profile runtimes (#607).

...
fn openai_compatible_model_catalog_refresh_calls_models_endpoint_and_updates_display() {
    ...
    assert!(
        request
            .to_ascii_lowercase()
            .contains("user-agent: omnis-key/0.1.0"),
...
#[path = "openrouter_sovereignty_tests.rs"]
...
#[test]
fn hostile_jcode_subscription_markers_cannot_restore_inherited_route_identity() {
    let _lock = ENV_LOCK.lock();
    let temp = TempDir::new().expect("create temp home");
    let jcode_home = temp.path().join("jcode-home");
    let _jcode_home = EnvVarGuard::set("JCODE_HOME", &jcode_home);
    ...
    let error = OpenRouterProvider::new()
        .err()
        .expect("inherited subscription transport must be refused");
    assert!(

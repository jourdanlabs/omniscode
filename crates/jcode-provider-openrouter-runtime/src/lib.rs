...
use jcode_provider_openrouter::{
    KIMI_FALLBACK_PROVIDERS, ModelsCache, ParsedProvider, PinSource, ProviderPin,
    current_unix_secs, known_providers, load_endpoints_disk_cache, parse_model_spec,
...
fn inherited_jcode_transport_requested() -> bool {
    [OPENROUTER_TRANSPORT_STATE_ENV, "JCODE_RUNTIME_PROVIDER"]
        .into_iter()
        .flat_map(std::env::var)
        .any(|value| {
            jcode_provider_core::sovereignty::is_inherited_jcode_subscription_alias(&value)
        })
}
...
pub fn maybe_schedule_openai_compatible_profile_catalog_refresh(
    _profile: jcode_base::provider_catalog::OpenAiCompatibleProfile,
    _context: &'static str,
) -> bool {
    // V1 is local-integrity first: merely rendering or constructing a provider
    // must never start catalog discovery, inspect credentials, or write cache
    // state. Network-backed catalog updates are available only through the
    // explicit `refresh_model_catalog` action.
    false
}
...
pub fn maybe_schedule_standard_openrouter_catalog_refresh(_context: &'static str) -> bool {
    // See the profile scheduler above. This compatibility entry point stays
    // registered so callers do not need a fork-specific code path, but it is
    // intentionally inert.
    false
}
...
impl OpenRouterProvider {
    ...
    fn maybe_schedule_endpoint_refresh(
        ...
        _context: &'static str,
        _notify_models_updated: bool,
    ) -> bool {
        false
    }

    fn maybe_schedule_model_catalog_refresh(&self, _cache_age_secs: u64, _context: &'static str) {
        // Automatic model discovery is disabled. Explicit catalog refresh is
        // implemented by `refresh_models` / `refresh_model_catalog`.
    }
    ...
    /// Read available models from process or disk cache without network access.
    ...
    /// Read per-provider endpoint data for a model from cache only.
    pub async fn fetch_endpoints(&self, model: &str) -> Result<Vec<EndpointInfo>> {
        ...
        // Check in-memory cache
        {
            ...
            if let Some((_cached_at, endpoints)) = cache.get(model) {

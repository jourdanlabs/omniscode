...
#[test]
fn construction_uses_ephemeral_machine_identity_without_home_writes() {
    let _guard = jcode_base::storage::lock_test_env();
    let temp = tempfile::tempdir().expect("temp home");
    let previous_home = std::env::var_os("HOME");
    let previous_jcode_home = std::env::var_os("JCODE_HOME");
    jcode_base::env::set_var("HOME", temp.path().to_string_lossy().to_string());
    jcode_base::env::set_var("JCODE_HOME", temp.path().to_string_lossy().to_string());
    let sentinel = temp.path().join("sentinel");
    std::fs::write(&sentinel, b"unchanged").expect("seed sentinel");

    let provider = CopilotApiProvider::new_with_token("fixture-token".to_string());
    assert_eq!(provider.machine_id.len(), 32);
    assert!(provider.machine_id.chars().all(|c| c.is_ascii_hexdigit()));
    assert_eq!(
    ...
    assert_eq!(
        std::fs::read_dir(temp.path())
            ...
            .count(),
        1,
        "provider construction must not create inherited machine-id state"
    );

    if let Some(value) = previous_home {
        jcode_base::env::set_var("HOME", value);
    } else {
        jcode_base::env::remove_var("HOME");
    }
    if let Some(value) = previous_jcode_home {
        jcode_base::env::set_var("JCODE_HOME", value);
    } else {

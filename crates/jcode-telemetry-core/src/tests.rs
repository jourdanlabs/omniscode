...
fn assert_no_session_state() {
    assert!(
        SESSION_STATE
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner())
            .is_none(),
        "disabled telemetry must not collect session state"
    );
}
...
#[test]
fn telemetry_is_permanently_disabled_for_omnis_v1() {
    let _guard = lock_test_env();
    jcode_core::env::remove_var("JCODE_NO_TELEMETRY");
...
#[test]
fn disabled_record_entries_preserve_conversion_id_and_write_nothing() {
    let _guard = lock_test_env();
    let prev_home = std::env::var_os("JCODE_HOME");
    let temp = tempfile::TempDir::new().expect("create temp dir");
    jcode_core::env::set_var("JCODE_HOME", temp.path());
    ...
    record_tool_call();
    record_tool_failure();
    record_connection_type("https");
    record_token_usage(1, 2, Some(3), Some(4));
    record_error(ErrorCategory::ToolError);
    record_provider_switch();
    record_model_switch();
    record_user_cancelled();
    record_todo_gate(TodoGateKind::Completion);
    record_tool_execution("bash", &serde_json::json!({"command": "true"}), true, 1);
    record_provider_selected("fixture");
    record_auth_started("fixture", "none");
    record_auth_failed("fixture", "none");
    record_auth_cancelled("fixture", "none");
    record_auth_surface_blocked("fixture", "none");
    record_auth_success("fixture", "none");
    record_feedback("fixture");
    record_command_family("/fixture");
    record_upgrade_if_needed();
    record_crash("fixture", "fixture", SessionEndReason::Disconnect);
    jcode_core::env::remove_var("JCODE_NO_TELEMETRY");
    assert_eq!(
        std::fs::read_to_string(&path).expect("conversion id remains"),
        "11111111-2222-4333-8444-555555555555\n"
    );
    assert!(
        !telemetry_id_path().expect("telemetry id path").exists(),
        "disabled record_turn must not create telemetry_id"
    );
    assert_eq!(
        std::fs::read_dir(temp.path())
            .expect("read controlled telemetry home")
            .count(),
        1,
        "disabled record functions must not create any additional state"
    );

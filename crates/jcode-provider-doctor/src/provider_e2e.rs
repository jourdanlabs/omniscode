...
impl NativeProviderKind {
    ...
    fn build_runtime(self) -> anyhow::Result<std::sync::Arc<dyn jcode_base::provider::Provider>> {
        ...
        let runtime: std::sync::Arc<dyn Provider> = match self {
            ...
            Self::Jcode => anyhow::bail!(
                "{}",
                jcode_base::subscription_catalog::INHERITED_SUBSCRIPTION_DISABLED_MESSAGE
            ),
            Self::Azure => {
...
mod tests {
    ...
    fn native_provider_kind_maps_every_generic_id() {
        ...
        assert_eq!(NativeProviderKind::from_normalized("jcode"), None);
    }
    ...
    #[test]
    fn native_jcode_doctor_runtime_refuses_before_construction() {
        let error = match NativeProviderKind::Jcode.build_runtime() {
            Ok(_) => panic!("inherited Jcode doctor runtime must refuse"),
            Err(error) => error,
        };
        assert_eq!(
            error.to_string(),
        ...
        assert_eq!(NativeProviderKind::from_normalized("jcode"), None);
        assert!(!native_doctor_supports_provider("jcode"));
    }

...
impl NotificationDispatcher {
    ...
    pub fn dispatch_cycle_summary(&self, transcript: &AmbientTranscript) {
        ...
        let priority = match transcript.pending_permissions {
            Some(0) => Priority::Default,
            Some(_) | None => Priority::High,
        };
...
fn format_cycle_body_safe(transcript: &AmbientTranscript) -> String {
    ...
    match transcript.pending_permissions {
        Some(0) => {}
        Some(pending) => lines.push(format!("{} permission request(s) pending", pending)),
        None => {
            lines.push("Permission state unreadable; pending request count is unknown".to_string())
        }
    }
...
fn format_cycle_body_detailed(transcript: &AmbientTranscript) -> String {
    ...
    match transcript.pending_permissions {
        ...
        None => {
            lines.push(String::new());
            lines.push(
                "**⚠ Permission state unreadable** — pending request count is unknown".to_string(),
            );
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_format_cycle_body_with_unreadable_permission_state() {
        let transcript = AmbientTranscript {
            session_id: "test_unknown_permissions".to_string(),
            started_at: chrono::Utc::now(),
            ended_at: Some(chrono::Utc::now()),
            status: crate::safety::TranscriptStatus::Complete,
            provider: "claude".to_string(),
        ...
        assert!(format_cycle_body_safe(&transcript).contains("count is unknown"));
        assert!(format_cycle_body_detailed(&transcript).contains("state unreadable"));
    }

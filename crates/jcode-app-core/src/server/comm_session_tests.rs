...
fn prepare_visible_spawn_session_cleans_session_when_launch_errors() {
    ...
    let worktree = tempfile::TempDir::new().expect("temp worktree");
    let mut prepared_session_id = None;
    ...
    let error = prepare_visible_spawn_session(
        ...
        |session_id, _cwd: &std::path::Path, _selfdev, _provider_key| {
            prepared_session_id = Some(session_id.to_string());
    ...
    let prepared_session_id =
        prepared_session_id.expect("launch callback should receive the prepared session");
    assert!(
        !crate::session::session_exists(&prepared_session_id),
        "failed visible launch should not leave orphan prepared sessions"
    );
    let startup_path = crate::storage::jcode_dir()
        .expect("jcode dir")
        .join(format!("client-input-{prepared_session_id}"));
    assert!(
        !startup_path.exists(),
        "failed visible launch should remove the prepared startup message"
    );

...
#[test]
fn handle_reload_refuses_canary_session_without_signal_or_peer_event() -> Result<()> {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().map_err(|e| anyhow!(e))?;
    let prev_runtime = std::env::var_os("JCODE_RUNTIME_DIR");
    crate::env::set_var("JCODE_RUNTIME_DIR", temp.path());
    ...
    rt.block_on(async {
        ...
        let refusal = events
            .recv()
            .await
            .ok_or_else(|| anyhow!("reload refusal event"))?;
        assert!(matches!(
            refusal,
            ServerEvent::Error {
                id: 7,
                ref message,
                ..
            } if message == crate::update::UPDATE_DISABLED_MESSAGE
        ));
        assert!(events.try_recv().is_err(), "refusal must emit one event");
        assert!(
            peer_events.try_recv().is_err(),
            "refusal must not notify peer sessions"
        );
        assert!(
            tokio::time::timeout(std::time::Duration::from_millis(50), rx.changed())
                .await
                .is_err(),
            "refusal must not queue a reload signal"
        );
        assert!(
            crate::server::recent_reload_state(std::time::Duration::from_secs(5)).is_none(),
            "refusal must not publish reload state"
        );
        Ok::<_, anyhow::Error>(())
    })?;
...
async fn handle_reload_does_not_wait_for_busy_agent_lock() -> Result<()> {
    ...
    assert!(
        ...
        "busy-agent refusal must use the frozen disabled message"
    );
    assert!(events.try_recv().is_err(), "refusal must emit one event");
    assert!(
        tokio::time::timeout(std::time::Duration::from_millis(50), rx.changed())

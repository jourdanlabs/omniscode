...
pub(super) fn refuse_disabled_reload(
    id: u64,
    client_event_tx: &tokio::sync::mpsc::UnboundedSender<crate::protocol::ServerEvent>,
) -> bool {
    if crate::update::updates_enabled() {
        return false;
    }
    let event = crate::protocol::ServerEvent::Error {
        id,
        message: crate::update::UPDATE_DISABLED_MESSAGE.to_string(),
        retry_after_secs: None,
    };
    if client_event_tx.send(event).is_err() {
        crate::logging::warn("Server: reload refusal client disconnected");
    }
    true
}
    ...
    loop {
        ...
        if !crate::update::updates_enabled() {
            crate::logging::warn(&format!(
                "Server: refused inherited reload signal request={}: {}",
                signal.request_id,

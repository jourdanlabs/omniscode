...
mod tests {
    ...
    #[tokio::test]
    async fn debug_reload_refuses_before_repository_lookup_or_state_writes() {
        let _env_lock = lock_env();
        let temp = tempfile::tempdir().expect("temp home");
        let _home = EnvGuard::set("JCODE_HOME", &temp.path().to_string_lossy());
        let _deferred = EnvGuard::set("JCODE_DEFERRED_AUTH_BOOTSTRAP", "1");
        ...
        let mut entries_before = std::fs::read_dir(temp.path())
            .expect("read fixture before refusal")
            .map(|entry| entry.expect("read fixture entry").file_name())
            .collect::<Vec<_>>();
        entries_before.sort();
        let error = execute_debug_command(
            agent,
            "reload",
        ...
        .await
        .expect_err("inherited debug reload must refuse");

        assert_eq!(error.to_string(), crate::update::UPDATE_DISABLED_MESSAGE);
        assert_eq!(
            std::fs::read(&sentinel).expect("read sentinel"),
            b"unchanged"
        );
        let mut entries_after = std::fs::read_dir(temp.path())
            .expect("read fixture after refusal")
            .map(|entry| entry.expect("read fixture entry").file_name())
            .collect::<Vec<_>>();
        entries_after.sort();
        assert_eq!(
            entries_after, entries_before,
            "disabled debug reload must not create build, manifest, symlink, or reload-info state"
        );
    }

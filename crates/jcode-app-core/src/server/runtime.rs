...
impl ServerRuntime {
    ...
    pub(super) async fn shutdown_when_accept_loop_exits(
        &self,
        mut main_handle: tokio::task::JoinHandle<()>,
        debug_handle: Option<tokio::task::JoinHandle<()>>,
    ) {
        if let Some(mut debug_handle) = debug_handle {
            tokio::select! {
                result = &mut main_handle => {
                    ...
                    if let Err(error) = debug_handle.await {
                        crate::logging::error(&format!("Debug accept loop join failed: {error}"));
                    }
                }
                result = &mut debug_handle => {
                    ...
                    if let Err(error) = main_handle.await {
                        crate::logging::error(&format!("Main accept loop join failed: {error}"));
                    }
                }
            }
        } else if let Err(error) = main_handle.await {
            crate::logging::error(&format!("Main accept loop failed: {error}"));
            self.shutdown().await;
        }
    }

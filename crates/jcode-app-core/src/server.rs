...
use self::minimal_startup::deferred_auth_bootstrap_active;
...
impl Server {
    ...
    pub fn new_with_name(provider: Arc<dyn Provider>, server_name: Option<String>) -> Self {
        ...
        let ambient_runner = minimal_startup::initialize_ambient_runner();
    ...
    async fn finish_startup_after_bind(
        ...
        debug_listener: Option<Listener>,
        server_start_time: Instant,
    ) -> (
        ServerRuntime,
        ...
        let debug_handle = debug_listener
            .map(|listener| runtime.spawn_debug_accept_loop(listener, server_start_time));
    ...
    fn spawn_background_tasks(
        ...
        if !deferred_auth_bootstrap_active() && crate::embedding::is_model_available() {
        ...
        } else if !deferred_auth_bootstrap_active() {
            crate::logging::info(
        ...
        if !deferred_auth_bootstrap_active() && crate::config::config().features.memory {
            tokio::spawn(async {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        let deferred_startup = deferred_auth_bootstrap_active();
        if !deferred_startup {
            crate::transport::remove_socket(&self.debug_socket_path);
        }

        let main_listener = Listener::bind(&self.socket_path)?;
        let debug_listener = if deferred_startup {
            None
        } else {
            Some(Listener::bind(&self.debug_socket_path)?)
        };
        ...
        #[cfg(unix)]
        {
            ...
            if let Some(listener) = debug_listener.as_ref() {
        ...
        runtime
            .shutdown_when_accept_loop_exits(main_handle, debug_handle)
            .await;
        Ok(())
    }

...
async fn test_background_command_without_timeout_keeps_running_past_default_foreground_timeout() {
    ...
    let mut final_status = None;
    // The command itself should take only 250ms, but observing its terminal
    // status also depends on process scheduling and the asynchronous status-file
    // write. Use the same wall-clock budget as the neighboring background tests
    // so a loaded parallel suite cannot exhaust a short poll count.
    let deadline = std::time::Instant::now() + std::time::Duration::from_secs(30);
    while std::time::Instant::now() < deadline {
        let status = crate::background::global()
            .status(&task_id)

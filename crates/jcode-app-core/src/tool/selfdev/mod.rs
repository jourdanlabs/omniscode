...
pub const JCODE_REPO_URL: &str = "https://github.com/jourdanlabs/omniscode.git";
...
struct SelfDevInput {
    ...
    /// Build target for selfdev build: auto, tui, or all. Every retained
    /// spelling builds the sole shipped `jcode` compatibility binary.
    #[serde(default)]
            ...
            json!({
                ...
                "properties": {
                    ...
                    "target": {
                        "type": "string",
                        "enum": ["auto", "tui", "all"],
                        "description": "Build target for action=build. Every retained value builds only the shipped jcode compatibility binary."
                    },
                    "command": {
                        "type": "string",

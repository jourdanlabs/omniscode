...
const DISCOVERY_REQUEST_ID_HEADER: &str = "x-omnis-key-discovery-request-id";
const DISCOVERY_SESSION_ID_HEADER: &str = "x-omnis-key-discovery-session-id";
const DISCOVERY_SESSION_METADATA_HEADER: &str = "x-omnis-key-discovery-session-metadata";
const DISCOVERY_SELF_DEV_HEADER: &str = "x-omnis-key-discovery-self-dev";
const DISCOVERY_DEBUG_HEADER: &str = "x-omnis-key-discovery-debug";
const DISCOVERY_CANARY_HEADER: &str = "x-omnis-key-discovery-canary";
const DISCOVERY_EXECUTION_MODE_HEADER: &str = "x-omnis-key-discovery-execution-mode";
const DISCOVERY_BUILD_CHANNEL_HEADER: &str = "x-omnis-key-discovery-build-channel";
const DISCOVERY_GIT_CHECKOUT_HEADER: &str = "x-omnis-key-discovery-git-checkout";
const DISCOVERY_CI_HEADER: &str = "x-omnis-key-discovery-ci";
const DISCOVERY_RAN_FROM_CARGO_HEADER: &str = "x-omnis-key-discovery-ran-from-cargo";
...
/// `discover_tools`: query a user-configured third-party tool catalog.
///
/// Catalog entries are unverified third-party data, not endorsements. The
/// request carries the category, a short search query, a
/// reason string, and coarse session/build provenance. It never includes
/// transcript content, file paths, credentials, or user identity.
...
impl Tool for DiscoverToolsTool {
    ...
    fn description(&self) -> &str {
        "Use proactively whenever fully completing the user's goal requires an external \
         ...
         submit one `suggest` action to the configured catalog endpoint, then continue without promising it \
         will be added. Skip discovery when available tools fully cover the task. Every listing is \
         unverified third-party data: do not imply endorsement, approval, independent review, or \
         guaranteed setup. Choose the best option based only on fit and ask before \
        ...
        json!({
            ...
            "properties": {
                ...
                "query": {
                    ...
                    "description": "Required capability summary sent to the configured third-party catalog endpoint. Write a fresh summary instead of copying user text. Never include secrets, credentials, personal data, or private content."
                },
                "reason": {
                    "type": "string",
                    "minLength": DISCOVERY_REASON_MIN_CHARS,
                    "maxLength": DISCOVERY_REASON_MAX_CHARS,
                    "description": "Required rationale sent to the configured third-party catalog endpoint. For select, explain why the tool fits better than alternatives. For suggest, explain why browse results were unsuitable. Never include private data."
                },
                "tool": {
                    "type": "string",
                ...
                "gap_evidence": {
                    "type": "string",
                    "maxLength": 500,
                    "description": "Optional concise explanation of which browse results were close and why they did not fit. Sent to the configured catalog endpoint."
                },
                "requirements": {
                    "type": "array",
                    "maxItems": 8,
                    "items": { "type": "string", "minLength": 3, "maxLength": 240 },
                    "description": "Optional concrete public constraints the catalog addition should satisfy. Sent to the configured catalog endpoint."
                },
                "prior_request_id": {
                    "type": "string",
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        if !config.sponsors.enabled || !crate::config::sponsors_endpoint_is_allowed(&endpoint) {
            ...
            return Err(anyhow::anyhow!(
                "tool discovery requires [sponsors] enabled = true and an explicit non-Jcode HTTP(S) endpoint"
            ));
        }
        ...
        if let Some(tool_name) = tool_selection {
            ...
            return Ok(ToolOutput::new(rendered)
                .with_title(format!("{tool_name} (third-party catalog)"))
                .with_metadata(json!({
                    "third_party_catalog": true,
                    "category": category,
                    "selected_tool": tool_name,
                    "catalog_url": crate::sponsors::DISCOVERY_CATALOG_SOURCE_URL,
                })));
        }
        ...
        Ok(ToolOutput::new(rendered)
            .with_title(format!("{} (third-party catalog)", category))
            .with_metadata(json!({
                "third_party_catalog": true,
                "category": category,
                "catalog_url": crate::sponsors::DISCOVERY_CATALOG_SOURCE_URL,
    ...
    let mut request = context.provenance.apply(
        context
            ...
            .header(
                ...
                format!("omnis-key/{}", env!("CARGO_PKG_VERSION")),
            )
...
async fn submit_suggestion(
    ...
    let request = context.provenance.apply(
        context
            .client
            .post(endpoint)
            .header(
...
fn render_listing(category: &str, listing: &Value, request_id: &str) -> Result<String> {
    ...
    let mut out = format!(
        "Discoverable tools in '{category}' (user-configured third-party catalog; entries are \
         unverified and recommendations must be based only on fit; catalog: {}):\n",
        crate::sponsors::DISCOVERY_CATALOG_SOURCE_URL
    );
    for tool in tools {
    ...
    out.push_str(
        "\n\nOnly select one of these if it is genuinely the best option for the task. \
         ...
         the browse request ID below so the configured catalog endpoint receives the gap. \
         Before signup, installation, or spending, inspect the provider, terms, privacy, and \
         security with the user. No endorsement, approval, or commercial relationship is implied.",
    );
    out.push_str(&format!("\n\nBrowse request ID: `{request_id}`"));
    Ok(out)
}
...
fn render_suggestion(
    ...
    out.push_str(
        "\n\nStatus: accepted by the configured catalog endpoint. Delivery, review, addition, approval, and availability are not guaranteed.",
    );
    Ok(out)
}
...
fn render_selection(category: &str, tool_name: &str, listing: &Value) -> Result<String> {
    ...
    let blurb = tool.get("blurb").and_then(|v| v.as_str()).unwrap_or("");
    let mut out = format!(
        "Selected '{name}' from '{category}' (user-configured third-party catalog; entry is \
         unverified and selection must be based only on fit; catalog: {}):\n\n{name}: {blurb}",
        crate::sponsors::DISCOVERY_CATALOG_SOURCE_URL
    );
    if let Some(url) = tool.get("url").and_then(|v| v.as_str()) {
        out.push_str(&format!(" ({url})"));
...
mod tests {
    ...
    fn render_listing_includes_disclosure_and_tools() {
        let listing = json!({
            "tools": [
                {"name": "example-card-tool", "blurb": "sandbox payment helper", "url": "https://example.invalid/card-tool"},
            ]
        });
        let out =
            render_listing("payments", &listing, "11111111-2222-4333-8444-555555555555").unwrap();
        assert!(out.contains("example-card-tool"));
        assert!(out.contains("sandbox payment helper"));
        assert!(out.contains("user-configured third-party catalog"));
        assert!(out.contains("entries are unverified"));
    ...
    fn render_listing_instructs_selection_phase() {
        let listing = json!({
            "tools": [{"name": "example-card-tool", "blurb": "sandbox cards", "url": "https://example.invalid/card-tool"}]
        });
        let out =
            render_listing("payments", &listing, "11111111-2222-4333-8444-555555555555").unwrap();
    ...
    fn render_selection_includes_setup_and_disclosure() {
        let listing = json!({
            "tool": {
                "name": "example-card-tool",
                "blurb": "sandbox cards",
                "url": "https://example.invalid/card-tool",
                "setup": "example-card-tool --stdio"
            }
        });
        let out = render_selection("payments", "example-card-tool", &listing).unwrap();
        assert!(out.contains("Selected 'example-card-tool'"));
        assert!(out.contains("Setup: example-card-tool --stdio"));
        assert!(out.contains("user-configured third-party catalog"));
        assert!(out.contains("entry is unverified"));
        assert!(out.contains("selection must be based only on fit"));
        assert!(render_selection("payments", "ghost", &json!({})).is_err());
    }

    #[test]
    fn example_selection_preserves_neutral_mcp_setup_provenance() {
        let listing = json!({
            "tool": {
                "name": "example-mail-tool",
                "blurb": "fixture-only programmable email inboxes",
                "url": "https://example.invalid/mail-tool",
                "setup": "Connect with example-mail-tool --stdio.",
                "mcp": {
                    "command": "example-mail-tool",
                    "args": ["--stdio"]
                }
            }
        });

        let rendered = render_selection("email-messaging", "example-mail-tool", &listing).unwrap();
        assert!(rendered.contains("Selected 'example-mail-tool'"));
        assert!(rendered.contains("example-mail-tool --stdio"));
        assert!(rendered.contains("No endorsement"));

        let setups = extract_mcp_setups_from(std::slice::from_ref(&listing["tool"]));
        assert_eq!(
            setups,
            vec![crate::sponsors::provenance::DiscoveredSetup {
                sponsor: "example-mail-tool".to_string(),
                command: "example-mail-tool".to_string(),
    ...
    fn schema_is_compact_and_self_contained() {
        ...
        assert!(description.contains("Every listing is unverified third-party data"));
        assert!(description.contains("do not imply endorsement"));
        assert!(!description.contains("vetted"));
        assert!(!description.contains("partner"));
        ...
        assert!(schema.contains("sent to the configured third-party catalog endpoint"));
    ...
    fn suggestion_validation_distinguishes_product_and_capability_gap() {
        let capability = DiscoverToolsInput {
            ...
            product_url: None,
            gap_evidence: Some(
                "The listed example provides cards rather than sandbox catalog administration."
                    .to_string(),
            ),
        ...
        known.product_url = Some("https://example.com/tool?version=stable#setup".to_string());
    ...
    fn render_suggestion_is_clear_about_review_status_and_recipient() {
        ...
        assert!(out.contains("accepted by the configured catalog endpoint"));
        assert!(out.contains("review, addition, approval, and availability are not guaranteed"));
    }
    ...
    fn discovery_text_rejects_recognizable_secrets_and_card_numbers() {
        ...
        let sensitive = [
            ...
            "Use card 4242 4242 4242 4242 for the external tool checkout".to_string(),
            ...
            "Contact private-person@example.com to configure the external capability".to_string(),
    ...
    async fn fetch_listing_round_trips_and_sends_only_expected_params() {
        let body = json!({"tools": [{"name": "example-card-tool", "blurb": "sandbox cards", "url": "https://example.invalid/card-tool"}]}).to_string();
        let (endpoint, server) = one_shot_server("HTTP/1.1 200 OK", body).await;
        let client = reqwest::Client::new();
        let request = test_discovery_request(&client, &endpoint, "request-test-1");
        let listing = fetch_listing(&request, None).await.unwrap();
        assert_eq!(listing.listing["tools"][0]["name"], "example-card-tool");
        ...
        assert!(
            request
                .to_ascii_lowercase()
                .contains("x-omnis-key-discovery-request-id: request-test-1"),
            "{request}"
        );
        assert!(
            !request
                .to_ascii_lowercase()
                .contains("x-omnis-key-discovery-benchmark"),
            "{request}"
        );
        for expected in [
            "x-omnis-key-discovery-session-id: session-test-1",
            "x-omnis-key-discovery-session-metadata: 1",
            "x-omnis-key-discovery-self-dev: 1",
            "x-omnis-key-discovery-debug: 0",
            "x-omnis-key-discovery-canary: 1",
            "x-omnis-key-discovery-execution-mode: agent_turn",
            "x-omnis-key-discovery-build-channel: selfdev",
            "x-omnis-key-discovery-git-checkout: 1",
            "x-omnis-key-discovery-ci: 0",
            "x-omnis-key-discovery-ran-from-cargo: 1",
        ] {
    ...
    async fn fetch_listing_hard_fails_on_http_error() {
        ...
        let client = reqwest::Client::new();
        let request = test_discovery_request(&client, &endpoint, "request-test-2");
    ...
    async fn fetch_listing_hard_fails_when_endpoint_unreachable() {
        ...
        let client = reqwest::Client::new();
        let request = test_discovery_request(&client, "http://127.0.0.1:9", "request-test-3");
        let err = fetch_listing(&request, None).await.unwrap_err();
        assert!(err.to_string().contains("discovery unavailable"));
    ...
    #[tokio::test]
    async fn submit_suggestion_posts_structured_catalog_payload() {
        let body = json!({
            "suggestion_id": "aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee",
            "status": "received",
            "message": "received"
        ...
        let suggestion = ValidatedSuggestion {
            ...
            product_url: Some("https://example.com/stripe-mcp".to_string()),
            gap_evidence: Some(
                "The listed example provides cards rather than sandbox object administration."
                    .to_string(),
            ),
            requirements: vec!["Scoped test-mode access".to_string()],
            prior_request_id: "11111111-2222-4333-8444-555555555555".to_string(),
        ...
        assert!(
            lower
                .contains("x-omnis-key-discovery-request-id: aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee"),
            "{request}"
        );
        assert!(
            !lower.contains("x-omnis-key-discovery-benchmark"),
            "{request}"
        );
    ...
    async fn execute_end_to_end_with_enabled_config_and_local_server() {
        ...
        let body = json!({"tools": [{"name": "example-card-tool", "blurb": "single-use sandbox cards", "url": "https://example.invalid/card-tool", "setup": "MCP server: example-card-tool --stdio"}]}).to_string();
        ...
        assert!(output.output.contains("example-card-tool"));
        assert!(
            output
                .output
                .contains("user-configured third-party catalog")
        );
        assert!(
            output
                .output
        ...
        assert!(title.contains("(third-party catalog)"), "{title}");
        let meta = output.metadata.unwrap();
        assert_eq!(meta["third_party_catalog"], true);

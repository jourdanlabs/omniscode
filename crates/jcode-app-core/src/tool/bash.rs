...
impl Tool for BashTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        // Fail closed: refuse to run if the ledger cannot accept a receipt.
        if let Err(e) = jcode_receipts::preflight_if_enabled() {
            return Err(e);
        }

        // Foreground execution with stdin detection
        let output = self.execute_foreground(&params, &ctx).await?;
        // Exit code is not always on ToolOutput; default 0 when the process
        // completed without an explicit non-zero surface in the formatted body.
        let exit = parse_exit_code_hint(&output.output).unwrap_or(0);
        receipt_bash(&params.command, exit, &ctx)?;
        Ok(output)
    }
}

fn parse_exit_code_hint(content: &str) -> Option<i32> {
    // format_command_output appends "\n[exit code: N]" for non-zero only.
...
fn receipt_bash(command: &str, exit_code: i32, ctx: &ToolContext) -> Result<()> {
    if !jcode_receipts::action_receipts_enabled() {
        return Ok(());
    }
    let cwd = super::receipt_util::cwd_from_ctx(&ctx.working_dir);
    jcode_receipts::record_bash(jcode_receipts::BashReceiptInput {
        command,
        exit_code,
        cwd: &cwd,
        repo_root: &cwd,

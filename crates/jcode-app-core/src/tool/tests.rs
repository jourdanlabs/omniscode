...
#[tokio::test]
async fn inherited_browser_tool_is_not_registered() {
    let provider: Arc<dyn Provider> = Arc::new(MockProvider);
    let registry = Registry::new(provider).await;
    ...
    assert!(
        ...
        "the inherited browser installer/bridge surface must not be advertised"
    );
}

#[tokio::test]
async fn deferred_startup_constructs_an_empty_tool_registry() {
    let _guard = crate::storage::lock_test_env();
    let previous = std::env::var_os("JCODE_DEFERRED_AUTH_BOOTSTRAP");
    ...
    assert!(
        ...
        "deferred startup must not construct tools whose constructors can initialize network or credential stores"
    );

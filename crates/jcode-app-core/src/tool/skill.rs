...
mod tests {
    ...
    async fn test_list_empty() {
        ...
        assert!(!result.output.contains("npx skills add"));
    }

    #[tokio::test]
    async fn test_load_unknown_skill_does_not_emit_install_hint() {
        let tool = create_test_tool();
        let ctx = create_test_context();
        let input = json!({"action": "load", "name": "frontend-design"});

        let error = tool.execute(input, ctx).await.unwrap_err().to_string();
        assert!(error.contains("not found"));
        assert!(!error.contains("install"));
        assert!(!error.contains("endorsed"));
    }

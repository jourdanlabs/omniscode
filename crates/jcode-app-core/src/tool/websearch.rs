...
mod tests {
    ...
    // Minimal synthetic fixtures exercise the parser branches without retaining
    // captured network responses.
    #[test]
    fn synthetic_ddg_anomaly_fixture_is_detected() {
    ...
    #[test]
    fn synthetic_ddg_results_fixture_parses() {
        let html = include_str!("testdata/ddg_results.html");
        assert_eq!(detect_anti_bot_page(html), None);
        assert!(
            !parse_ddg_results(html, 10).is_empty(),
            "synthetic results page should yield results"
        );
    }

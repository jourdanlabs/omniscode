...
#[cfg(unix)]
#[tokio::test]
async fn readiness_refuses_the_inherited_bridge_before_probing_a_stale_install() {
    ...
    let error = ensure_firefox_ready()
        .await
        .expect_err("inherited browser readiness must be disabled");
    let message = error.to_string();
    assert!(
        message.contains("OMNIS_KEY_INHERITED_BROWSER_DISABLED"),
        "{message}"
    );
    assert!(message.contains("disabled"), "{message}");
    assert!(!invocations.exists(), "hostile browser binary was executed");
    assert_eq!(
        std::fs::read_to_string(browser_dir.join(".setup-complete"))
            .expect("read unchanged setup marker"),
        "complete"
    );

...
impl Tool for EditTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        // Write first, then receipt; on receipt failure roll back (fail-closed).
        tokio::fs::write(&path, &new_content).await?;
        if let Err(receipt_err) =
            receipt_edit_or_rollback(&path, &params.file_path, &content, &new_content, &ctx).await
        {
            return Err(receipt_err);
        }
...
async fn receipt_edit_or_rollback(
    path: &Path,
    display_path: &str,
    before: &str,
    after: &str,
    ...
    let cwd = ctx.working_dir.clone().unwrap_or_else(|| {
        std::env::current_dir().unwrap_or_else(|_| Path::new(".").to_path_buf())
    });
    let target = super::receipt_util::repo_relative(&cwd, path, display_path);
    match jcode_receipts::record_edit(jcode_receipts::EditReceiptInput {
        target_repo_relative: &target,
        before,
        after,
        cwd: &cwd,
        repo_root: &cwd,
        ...
        Ok(_) => Ok(()),
        Err(e) => {
            // Fail closed: mutation must not remain unreceipted.

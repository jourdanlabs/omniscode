...
impl Tool for WriteTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        // Write the file, then receipt; roll back on receipt failure (fail-closed).
        tokio::fs::write(&path, &params.content).await?;
        if let Err(receipt_err) = receipt_write_or_rollback(
            &path,
            &params.file_path,
...
async fn receipt_write_or_rollback(
    path: &Path,
    display_path: &str,
    before: Option<&str>,
    after: &str,
    ...
    match jcode_receipts::record_write(jcode_receipts::WriteReceiptInput {

...
impl Registry {
    ...
    pub async fn new(_provider: Arc<dyn Provider>) -> Self {
        ...
        if std::env::var_os("JCODE_DEFERRED_AUTH_BOOTSTRAP").is_some() {
            // Deferred startup is a credential-free first-frame boundary. Do
            // not construct the ordinary tool set here: some tool
            // constructors eagerly build HTTP clients, which can initialize
            // the platform trust store and inspect user keychains even when
            // no request is sent. An explicit provider action starts a fresh
            // non-deferred runtime and receives the normal registry.
            let registry = Self::empty();
            crate::logging::info(&format!(
                "[TIMING] registry_new: deferred_empty=true, total={}ms",
        ...
        // Discovery is registered only after an explicit, sovereign endpoint
        // passes config validation.
        let sponsors = &crate::config::config().sponsors;
        if sponsors.enabled && crate::config::sponsors_endpoint_is_allowed(&sponsors.endpoint) {
            Self::insert_tool(
                &mut tools_map,
                "discover_tools",
    ...
    pub async fn register_mcp_tools_for_dir(
        ...
        if std::env::var_os("JCODE_DEFERRED_AUTH_BOOTSTRAP").is_some() {
            // Implicit first-run registration must not inspect or import global,
            // project, Claude, or Codex MCP configuration. The V1 startup shell
            // remains usable with its built-in local tools.
            return;
        }

...
const GITHUB_REPO: &str = "jourdanlabs/omniscode";
mod policy;
pub use policy::{UPDATE_DISABLED_MESSAGE, updates_enabled};
...
pub fn run_git_pull_ff_only(repo_dir: &Path, quiet: bool) -> Result<()> {
    if !updates_enabled() {
        anyhow::bail!(UPDATE_DISABLED_MESSAGE);
    }
...
#[cfg(test)]
#[path = "update_sovereignty_tests.rs"]

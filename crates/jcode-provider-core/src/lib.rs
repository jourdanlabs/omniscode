...
pub trait Provider: Send + Sync {
    ...
    ///
    /// Single runtimes may delegate to `set_model`; orchestrators must honor the exact runtime key.
    fn set_route_selection(&self, selection: &RouteSelection) -> Result<()> {
        selection.ensure_no_inherited_jcode_subscription()?;
    ...
    /// as route detail in the model picker (for example, "cached catalog").
    /// Empty when no detail is needed.
...
/// Canonical User-Agent for generic outbound OMNIS KEY HTTP requests.
pub const JCODE_USER_AGENT: &str = concat!("omnis-key/", env!("CARGO_PKG_VERSION"));
...
pub enum RouteCostSource {
    ...
    OpenRouterEndpoint,
    OpenRouterCatalog,
    /// Locally cached models.dev-format pricing data.
    ModelsDevCatalog,
    Heuristic,
}
...
mod tests {
    ...
    #[test]
    fn canonical_user_agent_identifies_omnis_key() {
        assert!(JCODE_USER_AGENT.starts_with("omnis-key/"));
    }

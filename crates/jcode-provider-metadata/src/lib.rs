...
fn login_providers_for_surface(surface: LoginProviderSurface) -> Vec<LoginProviderDescriptor> {
    let mut providers = login_providers()
        ...
        .copied()
        .filter(|provider| {
            provider.target != LoginProviderTarget::Jcode
...
pub fn resolve_login_provider(input: &str) -> Option<LoginProviderDescriptor> {
    ...
    login_providers()
        .iter()
        .copied()
        .filter(|provider| provider.target != LoginProviderTarget::Jcode)
        .find(|provider| {
...
mod tests {
    ...
    fn resolve_login_provider_loose_resolves_every_descriptor_by_id_and_display_name() {
        ...
        for descriptor in login_providers() {
            if descriptor.target == LoginProviderTarget::Jcode {
                assert_eq!(resolve_login_provider_loose(descriptor.id), None);
                assert_eq!(resolve_login_provider_loose(descriptor.display_name), None);
                continue;
            }
            assert_eq!(

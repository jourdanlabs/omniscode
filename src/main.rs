...
fn run_main() -> Result<()> {
    ...
    // Compatibility integrity commands are a process-entry boundary. Dispatch
    // them before allocator tuning, crypto-provider installation, runtime
    // construction, logging, configuration, hooks, providers, or any other
    // inherited startup work.
    if let Some(code) = omnis_key_cli::run_jcode_omnis_from(process_args.clone()) {
        std::process::exit(code);
    }

    // Root help/version and every parser refusal share the same process-entry
    // boundary. Besides keeping them ambient-free, this converts non-UTF-8
    // argv into a normal Clap usage refusal instead of allowing a later
    // `std::env::args()` conversion panic.
    if let Some(code) = jcode::cli::args::render_process_entry_parse_result(process_args.clone()) {
        std::process::exit(code);
    }
    ...
    // Retire previously installed SessionStart tracking hooks as inert success.
    // Keep the pre-runtime classification so stale Claude/Codex hook entries do
    // not enter normal startup, but never read or write setup-hint state.
    if cli_launch_hint_source(process_args.iter().skip(1)).is_some() {
        return Ok(());
    }

    // A stale macOS LaunchAgent may still invoke the retired listener after an
    // upgrade. Exit successfully before creating a runtime, entering a run
    // loop, reading launch plans, or spawning anything.
    if args_are_macos_hotkey_listener(process_args.iter().skip(1)) {
        return Ok(());
    }
...
fn args_are_macos_hotkey_listener<I, T>(args: I) -> bool
where
    I: IntoIterator<Item = T>,
    T: AsRef<OsStr>,
{
    let args: Vec<T> = args.into_iter().collect();
    args.first().and_then(|arg| arg.as_ref().to_str()) == Some("setup-hotkey")
        && args
            .iter()
            .any(|arg| arg.as_ref() == OsStr::new("--listen-macos-hotkey"))
}

fn cli_launch_hint_source<I, T>(args: I) -> Option<String>
where
    I: IntoIterator<Item = T>,
    T: AsRef<OsStr>,
{
    let args: Vec<T> = args.into_iter().collect();
    if args.first().and_then(|arg| arg.as_ref().to_str()) != Some("setup-hotkey") {
        return None;
    }
    let index = args
        .iter()
        .position(|arg| arg.as_ref() == OsStr::new("--notify-cli-launch"))?;
    args.get(index + 1)?.as_ref().to_str().map(str::to_string)
}

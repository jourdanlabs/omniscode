...
/// Render any root-parser help, version, or usage result before the executable
/// constructs its async runtime or initializes inherited subsystems.
///
/// Successful parses return `None` and are parsed again by normal startup.
/// Compatibility OMNIS commands are dispatched by `omnis-key-cli` before this
/// function is called because the canonical nested V1 grammar intentionally
/// supersedes the inherited parser.
pub fn render_process_entry_parse_result<I, T>(args: I) -> Option<i32>
where
    I: IntoIterator<Item = T>,
    T: Into<OsString> + Clone,
{
    let error = Args::try_parse_from(args).err()?;
    ...
    Some(if result.is_ok() { exit } else { 3 })
}
...
#[derive(Parser, Debug)]
#[command(name = "omnis-code")]
#[command(version = jcode_build_meta::version())]
#[command(about = "OMNIS CODE — a coding agent that proves what it did, or refuses to say")]
pub(crate) struct Args {
    ...
    /// Legacy compatibility flag. OMNIS KEY V1 never checks for updates.
    #[arg(long, global = true, hide = true)]
    pub(crate) no_update: bool,

    /// Legacy compatibility flag. OMNIS KEY V1 never installs updates.
    #[arg(long, global = true, default_value = "false", hide = true)]
    ...
    /// Explicit opt-out: do not append agent action receipts (edit/write/bash).
    /// Recorded in session summary — cannot be silent.
    #[arg(long, global = true)]
    pub(crate) no_receipts: bool,

    /// DANGEROUS: store full bash argv in the plaintext ledger (off by default).
    #[arg(long, global = true)]
    pub(crate) receipt_full_argv: bool,

    /// Also receipt read/ls/search tools (off by default — mutations only).
    #[arg(long, global = true)]
    pub(crate) receipt_reads: bool,
...
pub(crate) enum Command {
    ...
    /// Inherited Jcode account surface (disabled in OMNIS KEY V1)
    #[command(hide = true)]
    Account {
        #[command(subcommand)]
    ...
    /// Inherited updater (disabled in OMNIS KEY V1)
    #[command(hide = true)]
    Update,
    ...
    /// Record and verify bounded local OMNIS provenance receipts.
    Omnis {
        #[command(subcommand)]
        action: OmnisCommand,
    },
    ...
    /// Inherited client-pairing surface (disabled in OMNIS KEY V1)
    ...
    /// Disabled inherited hotkey installer retained only for parser compatibility
    #[command(hide = true)]
    SetupHotkey {
        /// Retired compatibility option; no macOS listener is started.
        #[arg(long, hide = true)]
        listen_macos_hotkey: bool,

        /// Retired compatibility option; no reminder or external hook is installed.
        #[arg(long, hide = true, value_name = "CLI")]
        notify_cli_launch: Option<String>,

        /// Retired compatibility option; no Windows listener is started.
        #[arg(long, hide = true)]
        listen_windows_hotkey: bool,

        /// Retired compatibility option; no installation or removal occurs.
        #[arg(long)]
        uninstall: bool,
    },

    /// Disabled inherited app-launcher installer retained only for parser compatibility
    #[command(hide = true)]
    SetupLauncher,

    /// Disabled inherited browser bridge installer/status surface retained for compatibility
    #[command(hide = true)]
    Browser {
        /// Retained compatibility action; the command refuses before browser work.
...
pub(crate) enum AccountCommand {
    /// Disabled inherited login surface
    Login {
        /// Ignored compatibility option
        #[arg(long, alias = "headless")]
        no_browser: bool,
    },
    /// Disabled inherited account-status surface
    Status {
        /// Emit JSON instead of human-readable output
        #[arg(long)]
        json: bool,
    },
    /// Disabled inherited account-management surface
    Manage,
    /// Disabled inherited account-logout surface
    Logout,
}

#[derive(Subcommand, Debug)]
pub(crate) enum OmnisCommand {
    /// Show the local receipt chain's current verification state.
    Status {
        /// Explicit receipt ledger path. Defaults to jcode's durable state directory.
        #[arg(long)]
    ...
    /// Verify the local receipt chain without recording a new event.
    Verify {
        /// Explicit receipt ledger path. Defaults to jcode's durable state directory.
        #[arg(long)]
        ledger: Option<String>,
    ...
    /// Flush durably committed safety-decision observations into the local receipt ledger.
    ReconcileSafety {
        /// Emit machine-readable JSON.
        #[arg(long)]
        json: bool,
    },
    /// Ask the separately activated checkpoint authority to retain a signed receipt-head snapshot.
    Anchor {
        /// Stable idempotency identity for exact same-anchor replay.
        #[arg(long)]
        request_id: String,
    ...
    /// Verify that the authority's latest signed checkpoint still exists in the local receipt chain.
    VerifyAnchored {
        /// Emit machine-readable JSON.
        #[arg(long)]
        json: bool,
    },
    /// Append a bounded local receipt. This records a supplied evidence digest; it does not execute an action.
    Record {
        /// Stable idempotency identity for exact same-event replay.
        #[arg(long)]
        event_id: String,

        /// Stable event kind, using ASCII letters, digits, '.', '-', or '_'.
        kind: String,

        /// Human-readable subject of the receipt.
        subject: String,

        /// SHA-256 digest of the cited evidence, with or without the sha256: prefix.
        #[arg(long)]
        evidence_sha256: String,

        /// Highest claim this entry may make: local-record or local-verification.
        #[arg(long, value_enum, default_value_t = OmnisClaimCeilingArg::LocalRecord)]
        claim_ceiling: OmnisClaimCeilingArg,
...
#[derive(Copy, Clone, Debug, Eq, PartialEq, ValueEnum)]
pub(crate) enum OmnisClaimCeilingArg {
    LocalRecord,
    LocalVerification,
}
...
pub(crate) enum ServerCommand {
    ...
    /// Inherited server replacement command (disabled in this distribution).
    #[command(hide = true)]
    Reload {
        /// Retained only for parser compatibility; the command always refuses.
        #[arg(long)]
        force: bool,

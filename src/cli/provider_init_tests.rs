...
#[test]
fn test_init_provider_jcode_refuses_before_runtime_profile_mutation() {
    let _guard = lock_env();
    let _env_guard = crate::storage::lock_test_env();
    let keys = [
    ...
    crate::env::set_var("JCODE_OPENROUTER_MODEL", "sentinel-model");
    crate::env::set_var("JCODE_RUNTIME_PROVIDER", "sentinel-runtime");
    crate::env::set_var("JCODE_ACTIVE_PROVIDER", "sentinel-active");
    crate::env::set_var("JCODE_INITIAL_PROVIDER_EXPLICIT", "sentinel-explicit");

    let runtime = tokio::runtime::Runtime::new().expect("tokio runtime");
    let error = match runtime.block_on(init_provider(&ProviderChoice::Jcode, None)) {
        Ok(_) => panic!("inherited Jcode subscription provider must remain disabled"),
        Err(error) => error,
    };
    ...
    assert_eq!(
        std::env::var(crate::subscription_catalog::JCODE_SUBSCRIPTION_ACTIVE_ENV)
            .ok()
            .as_deref(),
        Some("1")
    );
...
#[test]
fn test_login_and_bootstrap_jcode_refuses_before_login() {
    let _guard = lock_env();
    let _env_guard = crate::storage::lock_test_env();
    crate::subscription_catalog::clear_runtime_env();
    crate::env::set_var(
    ...
    let runtime = tokio::runtime::Runtime::new().expect("tokio runtime");
    let error = match runtime.block_on(login_and_bootstrap_provider(
        provider_catalog::JCODE_LOGIN_PROVIDER,
        None,
    )) {
        Ok(_) => panic!("inherited Jcode subscription login must remain disabled"),
        Err(error) => error,
    };
...
fn choice_for_login_provider_round_trips_core_targets() {
    ...
    assert_eq!(login_provider_for_choice(&ProviderChoice::Jcode), None);
    assert_eq!(
    ...
    for provider in provider_catalog::login_providers() {
        if matches!(
            ...
            provider_catalog::LoginProviderTarget::AutoImport
                | provider_catalog::LoginProviderTarget::Jcode
        ) {
...
fn auth_integration_registry_matches_cli_choice_runtime_wiring() {
    for provider in provider_catalog::login_providers() {
        if provider.target == provider_catalog::LoginProviderTarget::Jcode {
            assert!(
                crate::auth::integration::auth_provider_integration(provider.id).is_none(),
                "disabled inherited provider must have no auth integration"
            );
            continue;
        }
...
async fn auto_provider_does_not_select_config_default_provider() {
    let _guard = lock_env();
    let _env_guard = crate::storage::lock_test_env();
    let dir = TempDir::new().expect("temp dir");
    let saved: Vec<(String, Option<String>)> = [
...
"#,
    ...
    let provider = init_provider_for_validation(&ProviderChoice::Auto, None)
        .await
        .expect("auto startup should produce an inert provider");
    assert!(
        provider.model_routes().is_empty(),
        "auto startup must not select an ambient config provider"
    );
    assert_eq!(
        std::env::var("JCODE_DEFERRED_AUTH_BOOTSTRAP").as_deref(),
        Ok("1")
    );
...
async fn auto_provider_does_not_read_or_import_untrusted_external_auth() {
    let _guard = lock_env();
    let _env_guard = crate::storage::lock_test_env();
    let dir = TempDir::new().expect("temp dir");
    let saved: Vec<(String, Option<String>)> = [
    ...
    let provider = init_provider_for_validation(&ProviderChoice::Auto, None)
        .await
        .expect("auto init should remain credential-less");
    assert!(
        provider.model_routes().is_empty(),
        "auto mode must not import or activate an external credential"
    );
    let external_auth_after =
        std::fs::read_to_string(&opencode_path).expect("external auth sentinel remains readable");
    assert!(external_auth_after.contains("\"access\":\"oc_acc\""));
    assert!(
        !dir.path().join("auth.json").exists(),
        "auto startup must not import external credentials into JCODE_HOME"
    );
...
/// P1-A gate condition `HOLD-P1A-AMBIENT-ENV-ARMS-ROUTE`: an API key inherited
/// from the process environment is NOT an explicit provider selection. With no
/// login performed, auto must stay inert even though `auth status` will report
/// the key as available. Guards the README claims "no ambient credential
/// discovery on startup" and "auto startup stays empty until explicit
/// selection". If this fails, a shell profile or CI runner can silently arm a
/// live provider route.
#[tokio::test]
#[expect(
    clippy::await_holding_lock,
...
async fn auto_provider_does_not_arm_from_ambient_env_api_key() {
    let _guard = lock_env();
    let _env_guard = crate::storage::lock_test_env();
    let dir = TempDir::new().expect("temp dir");
    let keys = [
    ...
    // Ambient key present; no login has ever been performed, so the product
    // credential store is empty.
    crate::env::set_var("ANTHROPIC_API_KEY", "sk-ant-ambient-from-shell-profile");
    assert!(
        !dir.path().join("auth.json").exists(),
        "precondition: no login has been performed"
    );

    let provider = init_provider_for_validation(&ProviderChoice::Auto, None)
        .await
        .expect("auto init should stay credential-less");
    assert!(
        provider.model_routes().is_empty(),
        "ambient env API key must not arm a route without an explicit login"
    );

    // Regression: an unrelated leftover file in the product store must not be
    // mistaken for "the operator logged in". A stale bedrock.env from an old
    // experiment previously satisfied a store-presence check and let the ambient
    // ANTHROPIC_API_KEY above arm a live Anthropic route.
    let store = crate::storage::app_config_dir().expect("app config dir");
    std::fs::create_dir_all(&store).expect("create store dir");
    std::fs::write(
        store.join("bedrock.env"),
        "AWS_BEARER_TOKEN_BEDROCK=leftover-from-an-old-experiment\n",
    )
    .expect("write stale env file");
    ...
    assert!(
        provider.model_routes().is_empty(),
        "an unrelated file in the product store must not arm a route"
    );
...
/// P1-A: bare `--provider auto` must reach local jcode auth.json credentials.
#[tokio::test]
#[expect(
    clippy::await_holding_lock,
    reason = "test env locks intentionally stay held across provider init to isolate process-global auth env"
)]
async fn auto_provider_resolves_local_jcode_auth_credentials() {
    let _guard = lock_env();
    let _env_guard = crate::storage::lock_test_env();
    let dir = TempDir::new().expect("temp dir");
    let saved: Vec<(String, Option<String>)> = [
    ...
    crate::env::set_var("JCODE_HOME", dir.path());
    crate::env::set_var("JCODE_NON_INTERACTIVE", "1");
    // Simulate the bare-binary path: deferred is set before provider init.
    ...
    std::fs::write(
        ...
        serde_json::json!({
            "anthropic_accounts": [{
                "label": "claude-1",
                "access": "sk-ant-test-access-token-not-real",
                "refresh": "sk-ant-test-refresh-token-not-real",
                "expires": expires,
    ...
    .expect("write local auth.json");
    crate::auth::AuthStatus::invalidate_cache();
    crate::config::invalidate_config_cache();

    let provider = init_provider_for_validation(&ProviderChoice::Auto, None)
        .await
        .expect("auto should resolve local credentials");
    assert!(
        !provider.model_routes().is_empty(),
        "auto with local auth.json must expose model routes (first-run reachability)"
    );
    assert!(
        std::env::var_os("JCODE_DEFERRED_AUTH_BOOTSTRAP").is_none(),
        "deferred bootstrap must clear once local credentials arm a real provider"
    );

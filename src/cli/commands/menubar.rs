...
/// Retired compatibility hook for the inherited automatic menu bar helper.
///
/// The explicit `jcode menubar` command remains user-directed, but ordinary
/// startup must never spawn an auxiliary process or create singleton state.
...
mod tests {
    ...
    #[test]
    fn inherited_menubar_autospawn_hook_is_an_inert_zero_write_compatibility_call() {
        let _guard = crate::storage::lock_test_env();
        ...
        ensure_menubar_helper_running();

        assert!(
            !jcode_home.exists(),
            "automatic menubar compatibility hook wrote singleton state"
        );
        if let Some(previous_home) = previous_home {
        ...
        if let Some(previous_jcode_home) = previous_jcode_home {
            crate::env::set_var("JCODE_HOME", previous_jcode_home);
        } else {
            crate::env::remove_var("JCODE_HOME");
        }
    }

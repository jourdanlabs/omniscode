...
fn direct_help(args: &[&str]) -> String {
    let error = Args::try_parse_from(args).expect_err("direct help must short-circuit parsing");
    assert_eq!(
        error.kind(),
        clap::error::ErrorKind::DisplayHelp,
        "unexpected parser error for {args:?}: {error}"
    );
...
#[test]
fn inherited_installer_direct_help_is_explicitly_disabled() {
    let hotkey = direct_help(&["jcode", "setup-hotkey", "--help"]);
    assert!(hotkey.contains("Disabled inherited hotkey installer"));
    assert!(hotkey.contains("no installation or removal occurs"));
    assert!(!hotkey.contains("Set up the platform global hotkey"));

    let launcher = direct_help(&["jcode", "setup-launcher", "--help"]);
    assert!(launcher.contains("Disabled inherited app-launcher installer"));
    assert!(!launcher.contains("Install a launcher so jcode appears"));

    let browser = direct_help(&["jcode", "browser", "--help"]);
    assert!(browser.contains("Disabled inherited browser bridge installer/status surface"));
    assert!(browser.contains("refuses before browser work"));
    assert!(!browser.contains("Browser automation setup and status"));
}
...
#[test]
fn omnis_checkpoint_commands_expose_no_authority_override() {
    let anchor = Args::try_parse_from([
        "jcode",
        "omnis",
        "anchor",
        "--request-id",
    ...
    .expect("OMNIS anchor should parse");
    assert!(matches!(
        anchor.command,
        Some(Command::Omnis {
            action: OmnisCommand::Anchor {
                request_id,
                json: true
            }
    ...
    let verify = Args::try_parse_from(["jcode", "omnis", "verify-anchored", "--json"])
        .expect("OMNIS anchored verification should parse");
    assert!(matches!(
        verify.command,
        Some(Command::Omnis {
            action: OmnisCommand::VerifyAnchored { json: true }
        })
    ));
    ...
    assert!(
        ...
        .is_err(),
        "checkpoint authority paths must not be caller-selectable"
    );
}
...
fn model_list_subcommand_parses() {
    ...
    match args.command {
        Some(Command::Cloud(CloudCommand::Sessions {
            ...
            assert!(open);
            assert!(with_view);
            assert_eq!(jade.user_id, "fixture-user");
        }
        other => panic!("unexpected command: {:?}", other),
    }
}
...
fn cloud_sessions_subcommands_parse() {
    ...
    match args.command {
        Some(Command::Cloud(CloudCommand::Sessions {
            ...
            assert_eq!(user_id.as_deref(), Some("fixture-user"));

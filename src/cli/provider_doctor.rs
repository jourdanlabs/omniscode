...
pub async fn run_provider_doctor_command(
    ...
    let normalized_provider = crate::auth::lifecycle::normalized_auth_provider_id(Some(provider));
    if normalized_provider == Some("jcode") {
    ...
    if native_doctor_supports_provider(provider) {
        let report = match normalized_provider {

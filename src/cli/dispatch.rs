...
pub(crate) async fn run_main(mut args: Args) -> Result<()> {
    ...
    match args.command {
        ...
        Some(Command::Omnis { action }) => super::omnis::run_command(action)?,
...
async fn run_default_command(args: Args) -> Result<()> {
    ...
    if !explicit_provider_or_model {
        // Keep the client-side first-frame path on the same credential-free
        // contract as the auto-spawned server. TUI status widgets otherwise
        // instantiate inherited helpers (for example the memory sidecar) that
        // inspect ambient OAuth/API-key sources before the operator acts.
    ...
    let explicit_tool_options = args.tool_profile.is_some()
        ...
        || args.disabled_tools.is_some()
        || args.disable_base_tools;
    let startup_hints = if args.fresh_spawn || !explicit_provider_or_model {
        None
    } else {
...
pub(crate) async fn spawn_server(
    ...
    if matches!(provider_choice, ProviderChoice::Auto) {

...
- Provide a phased migration plan that fits the refactoring roadmap.
...
- product surfaces outside the main TUI binary: none in V1; the inherited
  desktop applications are quarantined from the source tree
...
- a future desktop client: post-V1 only; no desktop application target is
  retained in the V1 source tree.

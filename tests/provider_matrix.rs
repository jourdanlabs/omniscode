...
#[tokio::test]
async fn provider_matrix_auto_startup_preserves_profile_env_without_activation() -> Result<()> {
    ...
    for provider in providers {
        ...
        for competing_state in [
            ...
            // A launcher may preserve an explicitly selected profile across a
            // process boundary, but V1 auto startup must not activate it or
            // enumerate its model catalog until an explicit provider action.
            ...
            assert!(
                ...
                "auto startup must not activate an ambient compatible profile for {context}"
            );
        }
    }
...
#[tokio::test]
async fn provider_matrix_auto_startup_ignores_ambient_non_compatible_auth_state_space() -> Result<()>
{
    ...
    for provider in providers {
        ...
        if seed_non_compatible_auto_auth(provider) {
            ...
            // Ambient credentials and stale saved profiles must remain inert
            // during the minimal auto-startup path.
            assert_no_active_compatible_profile_lock(&context);
            assert!(
                runtime.model_routes().is_empty(),
                "auto startup must not enumerate ambient provider routes for {context}"
            );
        }
    }

...
fn live_models_contract_uses_models_endpoint_and_bearer_auth() -> Result<()> {
    ...
    let models = run_current_thread(provider.refresh_models())?;
...
#[test]
fn model_picker_cache_miss_stays_offline_until_explicit_refresh_updates_routes() -> Result<()> {
    let env = TestEnv::new()?;
    let server = spawn_models_server(
        2,
    ...
    run_current_thread(async {
        ...
        assert_eq!(
            ...
            0,
            "model-picker rendering must not schedule an ambient /models request"
        );
        ...
        assert!(
            ...
            "explicit refresh should update picker models; display={display:?}"
        );
        ...
        assert!(
            ...
            "explicit refresh should update model routes; routes={routes:?}"
        );
        Ok::<(), anyhow::Error>(())
    })?;
    ...
    assert_eq!(
        server.request_count.load(Ordering::SeqCst),
        1,
        "only the explicit refresh should issue a /models request"
    );
...
fn live_model_catalog_failure_keeps_static_and_selected_model_picker_fallbacks() -> Result<()> {
    ...
    let err =
        run_current_thread(provider.refresh_models()).expect_err("catalog refresh should fail");

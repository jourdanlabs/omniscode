...
fn with_isolated_safety_home<T>(test: impl FnOnce() -> T) -> T {
    let _env = setup_test_env().expect("create isolated safety environment");
    test()
}
...
fn test_safety_permission_flow() {
    with_isolated_safety_home(|| {
        ...
        let baseline = safety.pending_requests().unwrap().len();
        ...
        let result = safety.request_permission(req).unwrap();
        assert!(matches!(result, PermissionResult::Queued { .. }));

        let pending = safety.pending_requests().unwrap();
        ...
        assert_eq!(safety.pending_requests().unwrap().len(), baseline);
    });
}
...
fn test_safety_transcript() {
    with_isolated_safety_home(|| {
        ...
        safety.save_transcript(&transcript).unwrap();
    });
}
...
fn test_safety_summary_generation() {
    with_isolated_safety_home(|| {
        ...
        let summary = safety.generate_summary().unwrap();
